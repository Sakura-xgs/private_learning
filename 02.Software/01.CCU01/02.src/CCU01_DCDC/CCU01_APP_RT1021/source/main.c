...
int main(void)
{
    ...
    (void)xTaskCreate(&Save_Eeprom_Task,           "SAVE_EEPROM",          500U/4U,   		NULL,   SAVE_EEPROM_TASK_PRIO,      NULL);
...
static void Led_Task(void *pvParameters)
{
	...
	*SCB_CCR |= (1U << 4U);//打开除0报错
    ...
    for (;;)
    {
    	...
    	if ((hour >= g_sStorage_data.ucLight_on_time) || (hour < g_sStorage_data.ucLight_off_time))
    	{
