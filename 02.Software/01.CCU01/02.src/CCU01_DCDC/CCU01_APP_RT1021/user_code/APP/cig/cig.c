 ...
 * @return
 */
void CigAssistRelayClose(const Gun_Id_e eGun_id)
{
	...
	if (GUN_A == eGun_id)
	{
		if (1)
		{
			...
			my_printf(USER_INFO, "GUN_A notify CIG close 12V assist relay\n");
		}
		else
		{
			CigContrlDataInput(CIG_CLOSE, CLOSE_GUNA_24V_ASSIST);
			my_printf(USER_INFO, "GUN_A notify CIG close 24V assist relay\n");
		}
	}
	else
	{
		if (1)
		{
			...
			my_printf(USER_INFO, "GUN_B notify CIG close 12V assist relay\n");
		}
		else
		{
 ...
 * @return
 */
void CigRelayOpenContrl(const Gun_Id_e eGun_id)
{
	if (FALSE == GunIdValidCheck(eGun_id))
	{
		return ;
	...
	if (GUN_A == eGun_id)
	{
		...
		else
		{
			...
			//my_printf(USER_INFO, "GUN_A open CIG relay(24V)\n");
		}
	}
	else
	{
		if (1)
		{
			...
			//my_printf(USER_INFO, "GUN_B open CIG relay(12V)\n");
		}
		else
		{
			CigContrlDataInput(CIG_OPEN, OPEN_GUNB_LOCK_RELAY_AND_24V_ASSIST);
			//my_printf(USER_INFO, "GUN_B open CIG relay(24V)\n");
		}
	}
}
...
static void GB_SetGunStatus(CC1_State_e type, const Gun_Id_e eGun_id)
{
	...
	if ((type == CC1_ALREADY_CONNECT) || (type == CC1_NEW_CONNECT))
	{
		if (PLUGGED_IN != g_sGun_data[eGun_id].eConnect_status)
		{
			...
			my_printf(USER_INFO, "%s CC1 update status -> PLUGGED_IN\n", (eGun_id==GUN_A)?"GUN_A":"GUNB");
		}
	}
	else
	{
		if (UNPLUGGED != g_sGun_data[eGun_id].eConnect_status)
		{
			...
			my_printf(USER_INFO, "%s CC1 update status -> UNPLUGGED\n", (eGun_id==GUN_A)?"GUN_A":"GUNB");
		}
	}
}
