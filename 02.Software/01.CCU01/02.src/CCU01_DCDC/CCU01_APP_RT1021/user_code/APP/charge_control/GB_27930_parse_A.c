...
GB_Version_t g_sGB_Version[GUN_MAX_NUM] = {0};
__BSS(SRAM_OC) GB_Charging_Charger_Data_t g_sGB_Charger_data[GUN_MAX_NUM] = {0};
__BSS(SRAM_OC) GB_Charging_Bms_Data_t g_sGB_Bms_data[GUN_MAX_NUM] = {0};
TimerHandle_t g_sRec_BRM_timer[GUN_MAX_NUM] = {NULL, NULL};
...
void GB_BcpPrase(const U8 *ucBuf, const U8 ucLen, const Gun_Id_e eGun_id)
{
    ...
    g_sGun_data[eGun_id].sTemp.unMax_allow_cell_vol = g_sGB_Bms_data[eGun_id].sBCP.unMax_allow_cell_vol;
    ...
    //预充电压小于车端电池电压5V
    g_sGun_data[eGun_id].sTemp.unEV_target_vol = g_sGB_Bms_data[eGun_id].sBCP.unVehicle_check_vol - GB_PRECHARGE_VOL_DIFF;
    my_printf(USER_INFO, "%s: receive full BCP message:limit VOL = %d(0.1)V, target VOL = %d(0.1)V, cur = %d(0.1)A SOC = %d(0.1) temperature = %d(-50)\n",
    		eGun_id == GUN_A?"GUN_A":"GUN_B", g_sGun_data[eGun_id].sTemp.unEV_max_imd_vol, g_sGun_data[eGun_id].sTemp.unEV_target_vol,
...
void GB_BcsPrase(const U8 *ucBuf, const U8 ucLen, const Gun_Id_e eGun_id)
{
    ...
    //g_sGB_Bms_data[eGun_id].sBCS.unMax_cell_vol_and_group;
    g_sGun_data[eGun_id].sTemp.ucCurrent_soc = g_sGB_Bms_data[eGun_id].sBCS.ucCurrent_SOC;
    if (INVALID_REMAIEND_TIME != g_sGB_Bms_data[eGun_id].sBCS.unRemained_time)
    {
...
void GB_BsmMessageProcess(const GB_BSM_Data_t sBSM_data, const Gun_Id_e eGun_id)
{
	...
	if ((U8)FALSE != sBSM_data.ucCell_vol_state)
	{
		if (g_sGun_fault[eGun_id].sRecoverable_fault.sItem.unBms_cell_vol_Abnormal != TRUE)
		{
			GunAlarmSet(&g_sGun_fault[eGun_id].sRecoverable_fault.uiWhole_flag, BIT32_STRUCT, FAULT_BMS_CELL_VOL_ABNORMAL);
	    	my_printf(USER_ERROR, "%s: receive BSM cell vol Abnormal\n", (eGun_id == GUN_A)?"GUN_A":"GUN_B");
		}
		err_cnt++;
	}
	...
	g_sGun_data[eGun_id].sTemp.ucMax_cell_temp = sBSM_data.ucMax_cell_temp;
	g_sGun_data[eGun_id].sTemp.ucMin_cell_temp = sBSM_data.ucMin_cell_temp;
	...
	else
	{
		if ((U8)FALSE == sBSM_data.ucCharge_enable_state)
		{
			...
			{
				//充电暂停时间不超过10分钟
				if ((xTaskGetTickCount() - g_sGB_Charger_data[eGun_id].stop_start_tick) > (10U*60U*997U))
				{
					g_sBms_ctrl[eGun_id].sRecv_bms_timeout[BST].unRecv_cnt = 0;
					g_sBms_ctrl[eGun_id].sRecv_bms_timeout[BST].uiStart_tick = xTaskGetTickCount();
					g_sBms_ctrl[eGun_id].sRecv_bms_timeout[BSD].uiStart_tick = g_sBms_ctrl[eGun_id].sRecv_bms_timeout[BST].uiStart_tick;
...
void GB_BstMessageProcess(const GB_BST_Data_t sBST_data, const Gun_Id_e eGun_id)
{
	...
	else if ((U8)INVALID_STATUS == sBST_data.sStop_reason.sItem.ucReceive_cst)
	{
		//桩端主动停止
	}
	else if ((U8)VALID_STATUS == sBST_data.sStop_reason.sItem.ucReceive_cst)
	{
		//车端回复正常
		g_sGun_data[eGun_id].sTemp.eStop_Charge_type = REACH_TOTAL_SOC_TARGET;
	}
...
void GB_BemTimeoutProcess(const GB_BEM_Data_t sBEM_data, const Gun_Id_e eGun_id)
{
	...
	if ((U8)TRUE == sBEM_data.sTimeout.crm_00)
	{
		if (g_sGun_warn[eGun_id].sRecoverable_warn.sItem.CCU_CRM_00_timeout != TRUE)
		{
			...
			my_printf(USER_INFO, "%s: receive BEM error: CRM 00 is timeout!!\n", (eGun_id==GUN_A)?"GUN_A":"GUN_B");
		}
	}
	if ((U8)TRUE == sBEM_data.sTimeout.crm_AA)
	{
		if (g_sGun_warn[eGun_id].sRecoverable_warn.sItem.CCU_CRM_AA_timeout != TRUE)
		{
			...
			my_printf(USER_INFO, "%s: receive BEM error: CRM AA is timeout!!\n", (eGun_id==GUN_A)?"GUN_A":"GUN_B");
		}
	}
	if ((U8)TRUE == sBEM_data.sTimeout.cml_cts)
	{
		if (g_sGun_warn[eGun_id].sRecoverable_warn.sItem.CCU_CML_CTS_AA_timeout != TRUE)
		{
			...
			my_printf(USER_INFO, "%s: receive BEM error: CML/CTS is timeout!!\n", (eGun_id==GUN_A)?"GUN_A":"GUN_B");
		}
	}
	if ((U8)TRUE == sBEM_data.sTimeout.cro)
	{
		if (g_sGun_warn[eGun_id].sRecoverable_warn.sItem.CCU_CR0_timeout != TRUE)
		{
			...
			my_printf(USER_INFO, "%s: receive BEM error: CRO is timeout!!\n", (eGun_id==GUN_A)?"GUN_A":"GUN_B");
		}
	}
	if ((U8)TRUE == sBEM_data.sTimeout.ccs)
	{
		if (g_sGun_warn[eGun_id].sRecoverable_warn.sItem.CCU_CCS_timeout != TRUE)
		{
			...
			my_printf(USER_INFO, "%s: receive BEM error: CCS is timeout!!\n", (eGun_id==GUN_A)?"GUN_A":"GUN_B");
		}
	}
	if ((U8)TRUE == sBEM_data.sTimeout.cst)
	{
		if (g_sGun_warn[eGun_id].sRecoverable_warn.sItem.CCU_CST_timeout != TRUE)
		{
			...
			my_printf(USER_INFO, "%s: receive BEM error: CST is timeout!!\n", (eGun_id==GUN_A)?"GUN_A":"GUN_B");
		}
	}
	if ((U8)TRUE == sBEM_data.sTimeout.csd)
	{
		if (g_sGun_warn[eGun_id].sRecoverable_warn.sItem.CCU_CSD_timeout != TRUE)
		{
			...
			my_printf(USER_INFO, "%s: receive BEM error: CSD is timeout!!\n", (eGun_id==GUN_A)?"GUN_A":"GUN_B");
		}
	}
...
void GBCAN1Parse(const U32 uiCan_id, const U8 *pucBuf)
{
	...
	//        g_sGB_Bms_data[GUN_A].sBSD.unMax_bat_cell_vol;
	//        g_sGB_Bms_data[GUN_A].sBSD.unMin_bat_cell_vol;
