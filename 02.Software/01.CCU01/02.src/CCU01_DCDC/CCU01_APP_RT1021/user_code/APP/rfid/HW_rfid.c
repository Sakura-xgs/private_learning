...
static void HwRfidParse(const U8 *pucBuf, U16 unInput_len)
{
	//校验包格式长度
	CHECK_PTR_NULL_NO_RETURN(pucBuf);
	CHECK_MSG_LEN_NO_RETURN(unInput_len, HW_FRAME_FORMAT_LENGTH_BYTES);
	...
	if (HW_HEAD_SIGN == pucBuf[SIGN_INDEX])
	{
		if ((HW_SUCCESS_STATUS == pucBuf[RESPONSE_STATUS_INDEX]) || (HW_FAIL_STATUS == pucBuf[RESPONSE_STATUS_INDEX]))
		{
			...
			//校验数据长度
			CHECK_MSG_LEN_NO_RETURN(unData_len+HW_FRAME_FORMAT_LENGTH_BYTES, unInput_len);
