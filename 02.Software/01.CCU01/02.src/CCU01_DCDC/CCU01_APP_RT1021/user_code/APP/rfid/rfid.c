...
static U32 g_uiNow_tick_rfid = 0;
...
static void RfidDataProcess(void)
{
	...
	g_uiNow_tick_rfid = xTaskGetTickCount();
}
...
static void RfidComCheck(void)
{
	...
	if ((xTaskGetTickCount() - g_uiNow_tick_rfid) > DEVICE_TIMEOUT_MS)
	{
...
static void Rfid_Request_Task(void *pvParameter)
{
	...
	while(1)
	{
		...
		if (NULL != g_psRFID_data)
		{
			...
			if (ucHeart_cnt >= (RFID_HEART_PERIOD_MS/WORKING_STATUS_TASK_PERIOD_MS))
			{
