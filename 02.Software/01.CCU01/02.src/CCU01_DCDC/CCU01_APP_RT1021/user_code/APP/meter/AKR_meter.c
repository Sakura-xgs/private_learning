...
{
	static U8 ucCnt[GUN_MAX_NUM] = {0};

	if (ucAddr == METER_ADD_A)
	{
		if ((U16)TRUE == g_sSelf_check.sItem.meter_comm_flag_A)
		{
			//高速采集电压、电流、功率寄存器
			...
			//减少低速寄存器的读取频率
			if (ucCnt[GUN_A] > LOW_SPEED_REG_READ_FREQ)
			{
				vTaskDelay(50);
				//电量
	...
	{
		if ((U16)TRUE == g_sSelf_check.sItem.meter_comm_flag_B)
		{
			//高速采集电压、电流、功率寄存器
			...
			//减少低速寄存器的读取频率
			if (ucCnt[GUN_B] > LOW_SPEED_REG_READ_FREQ)
			{
				vTaskDelay(50);
				//电量
...
static void MeterVersionParse(void)
{
	if (AKR_READ_CMD == g_sAKR_receive_version.ucCmd)
	{
		if (METER_ADD_A == g_sAKR_receive_version.ucSlave_addr)
		{
			...
			//置位解析标志位为A电表
			g_sAKR_Meter.eDevice_switch = GUN_A;
			(void)snprintf(g_sAKR_Meter.sPublic_data[GUN_A].ucMeter_version, sizeof(g_sAKR_Meter.sPublic_data[GUN_A].ucMeter_version), "%d", g_sAKR_receive_version.unVersion);
		}
		else if (METER_ADD_B == g_sAKR_receive_version.ucSlave_addr)
		{
			...
			//置位解析标志位为B电表
			g_sAKR_Meter.eDevice_switch = GUN_B;
			(void)snprintf(g_sAKR_Meter.sPublic_data[GUN_B].ucMeter_version, sizeof(g_sAKR_Meter.sPublic_data[GUN_B].ucMeter_version), "%d", g_sAKR_receive_version.unVersion);
		}
		else
		{
...
static void MeterRequestDataParse(void)
{
	if (AKR_READ_CMD == g_sAKR_receive_data.ucCmd)
	{
		...
		S32 sTemp_vol = (gHexToFloat(g_sAKR_receive_data.cVol,0) * 100.0f);
		if (METER_ADD_A == g_sAKR_receive_data.ucSlave_addr)
		{
			...
			//置位解析标志位为A电表
			g_sAKR_Meter.eDevice_switch = GUN_A;
			g_sAKR_Meter.sPublic_data[GUN_A].uiDc_vol = (U32)((sTemp_vol > 0)?sTemp_vol:-sTemp_vol);
		...
		else if (METER_ADD_B == g_sAKR_receive_data.ucSlave_addr)
		{
			...
			//置位解析标志位为B电表
			g_sAKR_Meter.eDevice_switch = GUN_B;
			g_sAKR_Meter.sPublic_data[GUN_B].uiDc_vol = (U32)((sTemp_vol > 0)?sTemp_vol:-sTemp_vol);
