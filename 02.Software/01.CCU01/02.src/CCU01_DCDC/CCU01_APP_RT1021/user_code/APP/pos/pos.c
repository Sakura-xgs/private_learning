 ...
 * @param
 */
static BOOL PosCmdControl(const Gun_Id_e eGun_id)
{
	...
	if(g_psPOS_data->POS_Transaction[eGun_id].bHmi_TransactionStart_flag == TRUE)
	{
		...
		BfPosTransactionStart(eGun_id);
		return FALSE;
	}
	else if(g_psPOS_data->POS_Transaction[eGun_id].bHmi_TransactionCompletion_flag == TRUE)
	{
		g_psPOS_data->bEnable_heart_flag = FALSE;
		BfPosTransactionCompletion(eGun_id);
		return FALSE;
	}
	else if(g_psPOS_data->POS_Transaction[eGun_id].bHmi_TransactionReversal_flag == TRUE)
	{
		g_psPOS_data->bEnable_heart_flag = FALSE;
		BfPosTransactionReversal(eGun_id);
		return FALSE;
	}
	else if(g_psPOS_data->POS_Transaction[eGun_id].bHmi_GetCardInfo_flag == TRUE)
	{
		g_psPOS_data->bEnable_heart_flag = FALSE;
		BfPosGetCardInfo(eGun_id);
		return FALSE;
	}
	else if(g_psPOS_data->POS_Transaction[eGun_id].bHmi_Abort_flag == TRUE)
	{

		g_psPOS_data->bEnable_heart_flag = FALSE;
		BfPosTransactionAbort(eGun_id);
		return FALSE;
	}
	else
	{
...
static void PosStartCharge(void)
{
			...
			|| (g_psHMI_data->eGun_id == g_sStorage_data.sPublic_data[GUN_B].eGun_id))
	{
		...
		if (GunId < GUN_MAX_NUM)
		{
			...
			g_sGun_data[GunId].sTemp.eStart_Charge_type = POS_START_MODE;
...
static void PosStopCharge(void)
{
			...
			|| (g_psHMI_data->eGun_id == g_sStorage_data.sPublic_data[GUN_B].eGun_id))
	{
		...
		if (GunId < GUN_MAX_NUM)
		{
			g_sGun_data[GunId].sTemp.eStop_Charge_type = POS_STOP_MODE;
			g_sMsg_control.sGun_charge_control.ucFinish_id = g_psHMI_data->eGun_id;
			g_sGun_data[GunId].sTemp.ucAuth_type = STOP_CHARGE_AUTH;

			//POS结束本地鉴权，不发送鉴权请求，鉴权通过则直接停止充电
			StopChargingControl(GunId);
