...
void TcpClose(void)
{
	if (NULL != g_sTcp_semaphores.tcp_send_mutex)
	{
		//防止传输中close导致错误
...
static U32 CCU_Data0001(U8 *pucDst)
{
	...
	i++;
	//照明灯
	pucDst[i] = g_sStorage_data.ucLight_on_time;
	i++;
	pucDst[i] = g_sStorage_data.ucLight_off_time;
	i++;
	//预留
...
static U32 CCU_Data0004(U8 *pucDst)
{
	...
	i++;
	//最高允许温度
	pucDst[i] = (0U == g_sGun_data[GUN_A].sTemp.ucBms_max_allow_temp)?BMS_MSG_TEMP_OFFSET:g_sGun_data[GUN_A].sTemp.ucBms_max_allow_temp;
	i++;
	//车辆接口测量电压
	...
	i++;
	//单体蓄电池最高温度
	pucDst[i] = (0U == g_sGun_data[GUN_A].sTemp.ucMax_cell_temp)?BMS_MSG_TEMP_OFFSET:g_sGun_data[GUN_A].sTemp.ucMax_cell_temp;
	i++;
	//单体蓄电池最低温度
	pucDst[i] = (0U == g_sGun_data[GUN_A].sTemp.ucMin_cell_temp)?BMS_MSG_TEMP_OFFSET:g_sGun_data[GUN_A].sTemp.ucMin_cell_temp;
	i++;
	//充电允许状态
	...
	i++;
	//单体蓄电池最高温度
	pucDst[i] = (0U == g_sGun_data[GUN_B].sTemp.ucMax_cell_temp)?50U:g_sGun_data[GUN_B].sTemp.ucMax_cell_temp;
	i++;
	//单体蓄电池最低温度
	pucDst[i] = (0U == g_sGun_data[GUN_B].sTemp.ucMin_cell_temp)?50U:g_sGun_data[GUN_B].sTemp.ucMin_cell_temp;
	i++;
	//充电允许状态
