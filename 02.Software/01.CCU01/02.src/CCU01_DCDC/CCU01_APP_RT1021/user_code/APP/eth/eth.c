...
//	(void)GetSigVal(CCU_SET_SIG_ID_GUN_NUM, &iIp_addr);
//	//无效PCU编号
//	if ((iIp_addr > PCU_NUM_MAX) || (iIp_addr < PCU_NUM_MIN))
//	{
//		iIp_addr = SERVERIP_ADDR2;
//	}
//    IP4_ADDR(&ipaddr, SERVERIP_ADDR0, SERVERIP_ADDR1, iIp_addr, SERVERIP_ADDR3);
    IP4_ADDR(&ipaddr, SERVERIP_ADDR0, SERVERIP_ADDR1, SERVERIP_ADDR2, SERVERIP_ADDR3);
    ...
    while(1)
    {
        ...
        {
        	TcpClose();
            my_printf(USER_ERROR, "%s:%d client connect failed:%d.%d.%d.%d\n", __FILE__, __LINE__, SERVERIP_ADDR0, SERVERIP_ADDR1,
            		iIp_addr, SERVERIP_ADDR3);
            PileAlarmSet(ALARM_ID_PCU_COMM_LOST_ERROR);
            vTaskDelay(2000);
            continue;
        }
