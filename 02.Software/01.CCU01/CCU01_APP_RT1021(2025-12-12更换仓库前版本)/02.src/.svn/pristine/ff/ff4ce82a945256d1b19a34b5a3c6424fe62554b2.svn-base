...
static U32 CCU_Data0002(U8 *pucDst)
{
	...
	//故障/告警所属区域,bit位表示
	pucDst[i] = g_sLocalState.ucAlarm_flag;
	i++;
...
static U32 CCU_Data8020(U8 *pucDst, BOOL bFlag)
{
	...
	(void)memcpy(&pucDst[i], &g_sLocalState.psMsg_head->unMsg_id, sizeof(g_sLocalState.psMsg_head->unMsg_id));
	i+=sizeof(g_sLocalState.psMsg_head->unMsg_id);
...
static BOOL AnalysisMsgHead(U8 *pucBuf, U16 unBuf_len)
{
	...
	if ((HEAD_NUM_H == pucBuf[0]) && (HEAD_NUM_L == pucBuf[1]))
	{
		...
		{
			g_sLocalState.psMsg_head = (MessageHead_t *)(&pucBuf[2]);

			if (NULL != g_sLocalState.psMsg_head)
			{
				if (unBuf_len == (g_sLocalState.psMsg_head->unMsg_len + MSGBODY_OFFSET))
				{
					return TRUE;
				}
				else
...
void CommunicationParse(U8 *pucBuf, U16 unBuf_len)
{
	...
	if (NULL != g_sLocalState.psMsg_head)
	{
		...
		case S_GUN_AUTH_ANS_CMD:
			...
			AnalysisMsgbody8003(g_sLocalState.psMsg_head->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
...
//			TimerSingleStop(FINISH_CHARGE_TIMER);
//			TimerSingleStop(CHARGING_TIMER);
//			AnalysisMsgbody8012(g_sLocalState.psMsg_head->unMsg_len, pucBuf + MSGBODY_OFFSET);
//			//g_sMsg_control.sCharging_control.bSend_charging_data_flag = FALSE;
//			break;
		...
		case C_SECC_UP_LOG_ANS_CMD:
			...
			AnalysisMsgbody0015(g_sLocalState.psMsg_head->unMsg_len, pucBuf);
			break;
		case S_SERVERSTART_CHARGE_CMD:
			my_printf(USER_INFO, "receive server[server start charge]control\n");
			AnalysisMsgbody0020(g_sLocalState.psMsg_head->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_SERVERSTOP_CHARGE_CMD:
			my_printf(USER_INFO, "receive server[server stop charge]control\n");
			AnalysisMsgbody0021(g_sLocalState.psMsg_head->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_BOOK_CHARGE_CMD:
			my_printf(USER_INFO, "receive server[server book charge]control\n");
			AnalysisMsgbody0022(g_sLocalState.psMsg_head->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_AMOUNT_CMD:
			my_printf(USER_INFO, "receive server[server amount]control\n");
			AnalysisMsgbody0024(g_sLocalState.psMsg_head->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_GUN_CONTROL_CMD:
			my_printf(USER_INFO, "receive server[server control]control\n");
			AnalysisMsgbody0025(g_sLocalState.psMsg_head->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_SET_GUN_PARAM_CMD:
			my_printf(USER_INFO, "receive server[server config]control\n");
			AnalysisMsgbody0026(g_sLocalState.psMsg_head->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
//		case S_SERVER_QUERY_STATUS_CMD:
//			my_printf(USER_INFO, "receive server[server query]control\n");
//			AnalysisMsgbody0027(g_sLocalState.psMsg_head->unMsg_len, pucBuf + MSGBODY_OFFSET);
//			break;
		case S_CCU_UPDATE_CMD:
			//my_printf(USER_INFO, "receive server[ccu update]control\n");
			AnalysisMsgbody0028(g_sLocalState.psMsg_head->unMsg_len, pucBuf);
			break;
		case S_SECC_UPDATE_CMD:
			//my_printf(USER_INFO, "receive server[secc update]control\n");
			AnalysisMsgbody0029(g_sLocalState.psMsg_head->unMsg_len, pucBuf);
			break;
		case S_SECC_LOG_REQUEST_CMD:
			//my_printf(USER_INFO, "receive server[secc log request]control\n");
			AnalysisMsgbody0030(g_sLocalState.psMsg_head->unMsg_len, pucBuf);
			break;
		case S_SERVER_STATUS_CMD:
			//my_printf(USER_INFO, "receive server[server status]control\n");
			AnalysisMsgbody0032(g_sLocalState.psMsg_head->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			PileAlarmReset(ALARM_ID_PCU_COMM_LOST_ERROR);
			break;
		case S_SERVER_FAULT_INJECTION_CMD:
			my_printf(USER_INFO, "receive server[alarm set]control\n");
			AnalysisMsgbody0034(g_sLocalState.psMsg_head->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_SERVER_TEST_CONTROL_CMD:
			my_printf(USER_INFO, "receive server[test control]control\n");
			AnalysisMsgbody0035(g_sLocalState.psMsg_head->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_PRODUCTION_CONTROL_CMD:
			AnalysisMsgbody0036(g_sLocalState.psMsg_head->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_CIG_UPDATE:
			AnalysisMsgbody0037(g_sLocalState.psMsg_head->unMsg_len, &pucBuf[0]);
			break;
		default:
			my_printf(USER_ERROR, "%s:%d receive tcp server msg cmd error = %04x\n", __FILE__, __LINE__, g_sLocalState.psMsg_head->unMsg_cmd);
			break;
		}
	}
}
	...
	while (1)
	{
		...
		if (NULL != g_sAlarm_tigger_sem)
		{
			if (pdTRUE == xSemaphoreTake(g_sAlarm_tigger_sem, portMAX_DELAY))
			{
				...
				AlarmFillFunc();
				//告警/故障bit位填充
				g_sLocalState.ucAlarm_flag = SetAlarmFlagFunc();
				//立即上传心跳
				TcpSendControl(&g_sMsg_control.sHeart_control.bHeart_timeup_flag);
				my_printf(USER_INFO, "trigger alarm modify:alarm flag = %d GUN_A=%d GUN_B=%d\n", g_sLocalState.ucAlarm_flag, g_sGun_data[GUN_A].eGun_common_status,
						g_sGun_data[GUN_B].eGun_common_status);
