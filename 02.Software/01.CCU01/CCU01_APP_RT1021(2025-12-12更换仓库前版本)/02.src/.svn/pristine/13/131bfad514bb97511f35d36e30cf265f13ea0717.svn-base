...
static void GunAImdControl(void)
{
	...
	//空闲查询频率降低
	if (UNPLUGGED == g_sGun_data[GUN_A].eConnect_status)
	{
	...
	{
		if (IMD_ENABLE == g_psIMD_data->sPublic_data[GUN_A].eCheck_flag)//启动绝缘监测
		{
...
static void GunBImdControl(void)
{
	...
	//空闲查询频率降低
	if (UNPLUGGED == g_sGun_data[GUN_B].eConnect_status)
	{
		if (s_ucImd_cnt >= (IDLE_STATUS_TASK_PERIOD_MS/WORKING_STATUS_TASK_PERIOD_MS))
		{
	...
	{
		//启动绝缘监测
		if (IMD_ENABLE == g_psIMD_data->sPublic_data[GUN_B].eCheck_flag)
		{
