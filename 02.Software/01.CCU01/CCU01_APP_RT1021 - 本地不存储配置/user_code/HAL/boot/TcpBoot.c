...
BOOL blTcpCheckUpdateMsg(void)
{
	...
	if(FALSE == g_blTcpUpdataFuncEnFg)
	{
		if ((FALSE == CheckChargingStatus(GUN_A)) && (FALSE == CheckChargingStatus(GUN_B)))
		{
			...
			g_sGun_data[GUN_A].ucGun_special_status = STA_UPDATE;//通知充电逻辑部分
			g_sGun_data[GUN_B].ucGun_special_status = STA_UPDATE;//通知充电逻辑部分
	...
	switch(u16DataType)
	{
		...
		case TCP_UPDATE_END_CMD:		//结束包
			...
			if((u16MsgNo > 0U) && (u16DataLen <= 256U))
			{
				...
				g_sConnect_control.ucHeart_unansNum=0;//收到一帧数据给心跳清0 ，防止重新登录
...
void TcpUpdataTimeOutFunc(void)
{
	...
	g_sGun_data[GUN_A].ucGun_special_status = STA_NORMAL;//通知充电逻辑部分
	g_sGun_data[GUN_B].ucGun_special_status = STA_NORMAL;//通知充电逻辑部分
