...
static U32 g_uiNow_tick_cig = 0;
__BSS(SRAM_OC) U8 s_ucCig_data[UART_COMM_BUF_LEN] = {0};
__BSS(SRAM_OC) CIG_Upgrade_t CIG_Upgrade;
static BOOL g_bEnable_second_uart = FALSE;
static U8 ucCig_uart = GB_GUNA_UART;
...
static void CigDataParse(const U8 *pucBuf, U16 unInput_len)
{
	...
	g_uiNow_tick_cig = xTaskGetTickCount();
...
static void CigDataProcess(void)
{
	...
	u16RecByteNum = RecUartData(ucCig_uart, &s_ucCig_data[0], UART_COMM_BUF_LEN);
...
static void CigComCheck(void)
{
	...
	if ((xTaskGetTickCount() - g_uiNow_tick_cig) > DEVICE_TIMEOUT_MS)
	{
...
static void Cig_Request_Task(void * pvParameters)
{
	...
	while (1)
	{
//		if (TRUE == g_bEnable_second_uart)
//		{
//			ucCig_uart = GB_GUNB_UART;
//		}
		...
		{
			CigCOMControl(ucCig_uart, CIG_REALY_RESET);
		}
		...
		if (CIG_Upgrade.gblCigUpgradeStatus == FALSE)
		{
			...
			{
				CigCOMControl(ucCig_uart, CIG_READ_VERSION);
				//cig小板版本信息读取
			...
			else
			{
				...
				if (TRUE == g_sCig_write_control.bSend_flag)
				{
					...
					{
						CigCOMControl(ucCig_uart, CIG_REALY_CONTROL);
						vTaskDelay(50);
					}
					else
					{
				...
				//读取状态
				CigCOMControl(ucCig_uart, CIG_READ_STATUS);
			}
		}
...
static void Cig_Parse_GUN_Task(void * pvParameters)
{
	...
	while (1)
	{
		if (NULL != Uart_recBinarySemaphore(ucCig_uart))
		{
			err = xSemaphoreTake(Uart_recBinarySemaphore(ucCig_uart), portMAX_DELAY);
			if (pdPASS == err)
			{
				CigDataProcess();
			}
 ...
 * @return
 */
static void GB_SetGunStatus(CC1_STATE_t type, U8 ucGun_id)
{
	...
	{
		if ((U8)PLUGGED_IN != g_sGun_data[ucGun_id].ucConnect_status)
		{
			g_sGun_data[ucGun_id].ucConnect_status = (U8)PLUGGED_IN;
	...
	else
	{
		...
		{
			g_sGun_data[ucGun_id].ucConnect_status = (U8)UNPLUGGED;
			my_printf(USER_INFO, "CC1 update status -> UNPLUGGED\n", (ucGun_id==GUN_A)?"GUN_A":"GUNB");
		}
	}
}

static void CC1StatusCheck(U8 ucGun_id)
{
	if (FALSE == GunIdValidCheck(ucGun_id))
	{
		return ;
    ...
    //用户按下枪按钮
	if ((g_sGun_data[ucGun_id].unCC1_vol < g_sStorage_data.unCC1_12V_max) && (g_sGun_data[ucGun_id].unCC1_vol > g_sStorage_data.unCC1_12V_min))
	{
		s_ucCount_6V[ucGun_id] = 0;
		s_ucCount_4V[ucGun_id] = 0;
        s_ucConnect_step[ucGun_id] = GUN_PRESS_DOWN;
        GB_SetGunStatus(CC1_WAIT_CONNECT, ucGun_id);
        s_uiConnect_step_tick[ucGun_id] = uiNow_tick;
        //my_printf(USER_INFO, "%s check CC1 change 6v-->12V\n", (ucGun_id==GUN_A)?"GUN_A":"GUNB");
	}
	//用户松开枪按钮
	else if ((g_sGun_data[ucGun_id].unCC1_vol < g_sStorage_data.unCC1_6V_max) && (g_sGun_data[ucGun_id].unCC1_vol > g_sStorage_data.unCC1_6V_min))
	{
		s_ucCount_6V[ucGun_id]++;
		if (s_ucCount_6V[ucGun_id] > CC1_CHECK_TIMEOUT_CNT)
		{
			if (s_ucConnect_step[ucGun_id] == GUN_LOCK_RELEASE)
			{
				//超过时间没有变成4V，说明并没有插抢到车
				if (uiNow_tick - s_uiConnect_step_tick[ucGun_id] > 10000U)
				{
					s_ucConnect_step[ucGun_id] = IDLE_STATUS;
					s_uiConnect_step_tick[ucGun_id] = 0;
					my_printf(USER_INFO, "%s check CC1 change 12v-->6V\n", (ucGun_id==GUN_A)?"GUN_A":"GUNB");
				}
			}
			else if (s_ucConnect_step[ucGun_id] == GUN_PRESS_DOWN)
			{
				s_ucConnect_step[ucGun_id] = GUN_LOCK_RELEASE;
				s_uiConnect_step_tick[ucGun_id] = uiNow_tick;
			...
			{
				//空闲状态
				GB_SetGunStatus(CC1_DISCONNECT, ucGun_id);
				s_ucConnect_step[ucGun_id] = IDLE_STATUS;
...
//				}
	...
	//与车端BMS连接成功
	else if ((g_sGun_data[ucGun_id].unCC1_vol < g_sStorage_data.unCC1_4V_max) && (g_sGun_data[ucGun_id].unCC1_vol > g_sStorage_data.unCC1_4V_min))
	{
        if ((s_ucConnect_step[ucGun_id] == GUN_LOCK_RELEASE) || (s_ucConnect_step[ucGun_id] == GUN_PRESS_DOWN))
        {
            s_ucCount_4V[ucGun_id]++;
            if (s_ucCount_4V[ucGun_id] > CC1_CHECK_TIMEOUT_CNT)
            {
                s_ucCount_4V[ucGun_id] = 0;
                s_ucCount_6V[ucGun_id] = 0;
				s_ucConnect_step[ucGun_id] = IDLE_STATUS;
				GB_SetGunStatus(CC1_NEW_CONNECT, ucGun_id);
        ...
        else
        {
			...
			if (s_ucCount_4V[ucGun_id] > CC1_CHECK_TIMEOUT_CNT)
			{
				...
				GB_SetGunStatus(CC1_ALREADY_CONNECT, ucGun_id);
			}
        }
	}
	else
