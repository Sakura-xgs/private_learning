 ...
 * @return
 */
void GB_BrmPrase(const U8 *ucBuf, const U8 ucLen, const U8 ucGun_id)
{
    ...
    (void)memcpy(&g_sGB_Bms_data[ucGun_id].sBRM, ucBuf, ucLen);

    //2015版
    if (g_sGB_Bms_data[ucGun_id].sBRM.ucBms_version[0] == 0x01U)
    {
    	g_sGB_Version[ucGun_id] = GB_2015;
    	(void)memcpy(g_sGun_data[ucGun_id].sTemp.ucBms_version, "1.1", 3);
    }
    //2023版
    else if (g_sGB_Bms_data[ucGun_id].sBRM.ucBms_version[0] == 0x02U)
    {
    	g_sGB_Version[ucGun_id] = GB_2023;
    	(void)memcpy(g_sGun_data[ucGun_id].sTemp.ucBms_version, "1.2", 3);
    }
    else
    {
    	//未知的充电协议
    ...
    g_sGun_data[ucGun_id].sTemp.ucBattery_type = g_sGB_Bms_data[ucGun_id].sBRM.ucBattery_type;
    //车端传递VIN才解析
    if (ucLen > 25U)
    {
    	(void)memcpy(g_sGun_data[ucGun_id].sTemp.ucVin, g_sGB_Bms_data[ucGun_id].sBRM.VIN, sizeof(g_sGB_Bms_data[ucGun_id].sBRM.VIN));
    }
    else
    {
    	(void)memset(g_sGun_data[ucGun_id].sTemp.ucVin, 0, sizeof(g_sGB_Bms_data[ucGun_id].sBRM.VIN));
    }

    g_sBms_ctrl[ucGun_id].sRecv_bms_timeout[BRM].unRecv_cnt++;
}
 ...
 * @return
 */
void GB_BcpPrase(const U8 *ucBuf, const U8 ucLen, const U8 ucGun_id)
{
	if (FALSE == GunIdValidCheck(ucGun_id))
	{
		return ;
    ...
    (void)memcpy(&g_sGB_Bms_data[ucGun_id].sBCP, ucBuf, sizeof(GB_BCP_Data_t));

    g_sGun_data[ucGun_id].sTemp.unSingle_bat_max_vol = g_sGB_Bms_data[ucGun_id].sBCP.unSingle_battery_max_allow_vol;
    g_sGun_data[ucGun_id].sTemp.unEV_max_allow_cur = 4000U-g_sGB_Bms_data[ucGun_id].sBCP.unMax_allow_cur;
    g_sGun_data[ucGun_id].sTemp.unBat_total_energy = g_sGB_Bms_data[ucGun_id].sBCP.unBattery_total_energy;
    g_sGun_data[ucGun_id].sTemp.unEV_max_limit_vol = g_sGB_Bms_data[ucGun_id].sBCP.unMax_allow_total_vol;
    g_sGun_data[ucGun_id].sTemp.ucBms_max_allow_temp = g_sGB_Bms_data[ucGun_id].sBCP.ucBattery_max_allow_temp;
    g_sGun_data[ucGun_id].sTemp.unStart_soc = g_sGB_Bms_data[ucGun_id].sBCP.unBattery_SOC;
    g_sGun_data[ucGun_id].sTemp.unVehicle_check_vol = g_sGB_Bms_data[ucGun_id].sBCP.unVehicle_check_vol;
    //预充电压设置为车端电压小5V
    g_sGun_data[ucGun_id].sTemp.unEV_target_vol = g_sGB_Bms_data[ucGun_id].sBCP.unVehicle_check_vol - 50U;
    my_printf(USER_INFO, "%s: receive full BCP message:limit VOL = %d(0.1)V, target VOL = %d(0.1)V, cur = %d(0.1)A SOC = %d(0.1) temperature = %d(-50)\n",
    		ucGun_id == GUN_A?"GUN_A":"GUN_B", g_sGun_data[ucGun_id].sTemp.unEV_max_imd_vol, g_sGun_data[ucGun_id].sTemp.unVehicle_check_vol,
			g_sGun_data[ucGun_id].sTemp.unEV_max_allow_cur, g_sGun_data[ucGun_id].sTemp.unStart_soc, g_sGB_Bms_data[ucGun_id].sBCP.ucBattery_max_allow_temp);
 ...
 * @return
 */
void GB_BcsPrase(const U8 *ucBuf, const U8 ucLen, const U8 ucGun_id)
{
	if (FALSE == GunIdValidCheck(ucGun_id))
	{
		return ;
    ...
    (void)memcpy(&g_sGB_Bms_data[ucGun_id].sBCS, ucBuf, ucLen);

    g_sGun_data[ucGun_id].sTemp.unVehicle_check_vol = g_sGB_Bms_data[ucGun_id].sBCS.unEV_detect_vol;
    g_sGun_data[ucGun_id].sTemp.unVehicle_check_cur = 4000U-g_sGB_Bms_data[ucGun_id].sBCS.unEV_detect_cur;
    //g_sGB_Bms_data[ucGun_id].sBCS.unMax_single_vol_and_group;
    g_sGun_data[ucGun_id].sTemp.ucCurrent_soc = g_sGB_Bms_data[ucGun_id].sBCS.ucCurrent_SOC;
    g_sGun_data[ucGun_id].sTemp.unRemain_time = g_sGB_Bms_data[ucGun_id].sBCS.unRemained_time;
    ...
    if (s_ucBsc_cnt > 40U)
    {
    	my_printf(USER_INFO, "%s: receive BCS message SOC = %d VOL = %d(0.1V) cur = %d(0.1A)\n", (ucGun_id == GUN_A?"GUN_A":"GUN_B"),
    			g_sGB_Bms_data[ucGun_id].sBCS.ucCurrent_SOC, g_sGun_data[ucGun_id].sTemp.unVehicle_check_vol
				,g_sGun_data[ucGun_id].sTemp.unVehicle_check_cur);
    	s_ucBsc_cnt = 0;
    }
}
 ...
 * @return
 */
void GB_BroMessageProcess(const U8 ucBro_status, const U8 ucGun_id)
{
	if (FALSE == GunIdValidCheck(ucGun_id))
	{
		return ;
 ...
 * @return
 */
void GB_BclMessageProcess(const GB_BCL_Data_t sBCL_data, const U8 ucGun_id)
{
	if (FALSE == GunIdValidCheck(ucGun_id))
	{
		return ;
	...
	g_sGun_data[ucGun_id].sTemp.unEV_target_cur	= 4000U-sBCL_data.unTarget_cur;
	//触发式上传需求电流，增加响应速度
	if (TRUE == my_unsigned_abs(g_sGun_data[ucGun_id].sTemp.unEV_target_cur, unTemp_cur, TARGET_CUR_DIFF))
	{
		my_printf(USER_INFO, "%s receive BCL target CUR change: %d-->%d(0.1A)\n", (ucGun_id == GUN_A?"GUN_A":"GUN_B"),
				unTemp_cur, g_sGun_data[ucGun_id].sTemp.unEV_target_cur);

		s_Send_flag = TRUE;
	}

	U16 unTemp_vol = g_sGun_data[ucGun_id].sTemp.unEV_target_vol;
	if (sBCL_data.unTarget_vol < (g_sStorage_data.sPublic_data[ucGun_id].unGun_max_vol * 10U))
	{
		g_sGun_data[ucGun_id].sTemp.unEV_target_vol = sBCL_data.unTarget_vol;
	}
	else
	{
	...
	// 基于最终生效的值判断是否触发上传
	if (TRUE == my_unsigned_abs(g_sGun_data[ucGun_id].sTemp.unEV_target_vol, unTemp_vol, TARGET_VOL_DIFF))
	{
	    my_printf(USER_INFO, "%s receive BCL target VOL change: %d-->%d(0.1V) limit VOL = %d(0.1V)\n",
	              (ucGun_id == GUN_A?"GUN_A":"GUN_B"),
				  unTemp_vol,
	              g_sGun_data[ucGun_id].sTemp.unEV_target_vol,
	              g_sGun_data[ucGun_id].sTemp.unEV_max_limit_vol);
	    s_Send_flag = TRUE;
	}
	...
	if (s_ucBcl_print_cnt[ucGun_id] > 200U)
	{
		my_printf(USER_INFO, "%s receive BCL: charge_type = %s VOL = %d(0.1V) cur = %d(0.1A)\n", (ucGun_id == GUN_A?"GUN_A":"GUN_B"),
				(sBCL_data.ucCharge_mode==1U)?"CV":"CC", g_sGun_data[ucGun_id].sTemp.unEV_target_vol, g_sGun_data[ucGun_id].sTemp.unEV_target_cur);
		s_ucBcl_print_cnt[ucGun_id] = 1;
	}
 ...
 * @return
 */
void GB_BsmMessageProcess(const GB_BSM_Data_t sBSM_data, const U8 ucGun_id)
{
	if (FALSE == GunIdValidCheck(ucGun_id))
	{
		return ;
	...
	{
		if (g_sGun_fault[ucGun_id].sRecoverable_fault.sItem.unBms_single_vol_Abnormal != TRUE)
		{
			GunAlarmSet(&g_sGun_fault[ucGun_id].sRecoverable_fault.uiWhole_flag, BIT32_STRUCT, FAULT_BMS_SINGLE_VOL_ABNORMAL);
	...
	{
		if (g_sGun_fault[ucGun_id].sRecoverable_fault.sItem.unBms_SOC_Abnormal != TRUE)
		{
			GunAlarmSet(&g_sGun_fault[ucGun_id].sRecoverable_fault.uiWhole_flag, BIT32_STRUCT, FAULT_BMS_SOC_ABNORMAL);
	...
	{
		if (g_sGun_fault[ucGun_id].sRecoverable_fault.sItem.unBms_over_cur != TRUE)
		{
			GunAlarmSet(&g_sGun_fault[ucGun_id].sRecoverable_fault.uiWhole_flag, BIT32_STRUCT, FAULT_BMS_OVER_CUR);
	...
	{
		if (g_sGun_fault[ucGun_id].sRecoverable_fault.sItem.unBms_bat_over_temp != TRUE)
		{
			GunAlarmSet(&g_sGun_fault[ucGun_id].sRecoverable_fault.uiWhole_flag, BIT32_STRUCT, FAULT_BMS_BAT_OVER_TEMP);
	...
	{
		if (g_sGun_fault[ucGun_id].sRecoverable_fault.sItem.unBms_insulation_error != TRUE)
		{
			GunAlarmSet(&g_sGun_fault[ucGun_id].sRecoverable_fault.uiWhole_flag, BIT32_STRUCT, FAULT_BMS_INSULATION_ERROR);
	...
	{
		if (g_sGun_fault[ucGun_id].sRecoverable_fault.sItem.unBms_charge_conn_error != TRUE)
		{
			GunAlarmSet(&g_sGun_fault[ucGun_id].sRecoverable_fault.uiWhole_flag, BIT32_STRUCT, FAULT_BMS_CHARGE_CONN_ERROR);
			my_printf(USER_ERROR, "%s: receive BSM connector Abnormal\n", (ucGun_id == GUN_A)?"GUN_A":"GUN_B");
		}
		err_cnt++;
	}

	g_sGun_data[ucGun_id].sTemp.ucSingle_bat_max_temp = sBSM_data.ucMax_battery_temp;
	g_sGun_data[ucGun_id].sTemp.ucSingle_bat_min_temp = sBSM_data.ucMin_battery_temp;
	...
	else
	{
		if ((U8)FALSE == sBSM_data.ucCharge_enable_state)
		{
			...
			{
				g_sGB_Charger_data[ucGun_id].stop_start_tick = xTaskGetTickCount();
			...
			{
				//充电暂停时间不超过10分钟
				if ((xTaskGetTickCount() - g_sGB_Charger_data[ucGun_id].stop_start_tick) > (10U*60U*996U))
				{
					g_sBms_ctrl[ucGun_id].sRecv_bms_timeout[BST].unRecv_cnt = 0;
					g_sBms_ctrl[ucGun_id].sRecv_bms_timeout[BST].uiStart_tick = xTaskGetTickCount();
					g_sBms_ctrl[ucGun_id].sRecv_bms_timeout[BSD].uiStart_tick = xTaskGetTickCount();
 ...
 * @return
 */
void GB_BstMessageProcess(const GB_BST_Data_t sBST_data, const U8 ucGun_id)
{
	if (FALSE == GunIdValidCheck(ucGun_id))
	{
		return ;
	...
	if ((U8)INVALID_STATUS == sBST_data.sStop_reason.sItem.ucAchieve_target_soc)
	{
        ...
        //填充CST
        g_sGB_Charger_data[ucGun_id].sCST.sStop_reason.sItem.ucReach_set_target = (U8)TRUE;
	}
	else if ((U8)INVALID_STATUS == sBST_data.sStop_reason.sItem.ucAchieve_target_vol)
	{
		g_sGun_data[ucGun_id].sTemp.ucStop_Charge_type = REACH_TOTAL_VOL_TARGET;
        //填充CST
        g_sGB_Charger_data[ucGun_id].sCST.sStop_reason.sItem.ucReach_set_target = (U8)TRUE;
	}
	else if ((U8)INVALID_STATUS == sBST_data.sStop_reason.sItem.ucTarget_single_vol)
	{
		g_sGun_data[ucGun_id].sTemp.ucStop_Charge_type = REACH_SINGLE_VOL_TARGET;
        //填充CST
        g_sGB_Charger_data[ucGun_id].sCST.sStop_reason.sItem.ucReach_set_target = (U8)TRUE;
	}
	...
	else
	{
		...
		{
			if (g_sGun_fault[ucGun_id].sRecoverable_fault.sItem.unBms_conn_over_temp != TRUE)
			{
				GunAlarmSet(&g_sGun_fault[ucGun_id].sRecoverable_fault.uiWhole_flag, BIT32_STRUCT, FAULT_BMS_CONN_OVER_TEMP);
			}
		}
		else if ((U8)INVALID_STATUS == sBST_data.sFault_reason.sItem.unBms_cell_over_temp)
		{
			if (g_sGun_fault[ucGun_id].sRecoverable_fault.sItem.unBms_cell_over_temp != TRUE)
			{
				GunAlarmSet(&g_sGun_fault[ucGun_id].sRecoverable_fault.uiWhole_flag, BIT32_STRUCT, FAULT_BMS_CELL_OVER_TEMP);
			}
		}
		...
		{
			if (g_sGun_fault[ucGun_id].sRecoverable_fault.sItem.unBms_relay_error != TRUE)
			{
				GunAlarmSet(&g_sGun_fault[ucGun_id].sRecoverable_fault.uiWhole_flag, BIT32_STRUCT, FAULT_BMS_RELAY_ERROR);
			}
		}
		...
		{
			if (g_sGun_fault[ucGun_id].sRecoverable_fault.sItem.unBms_other_fault != TRUE)
			{
		...
		{
			if (g_sGun_fault[ucGun_id].sRecoverable_fault.sItem.unBms_check_p2_err != TRUE)
			{
				GunAlarmSet(&g_sGun_fault[ucGun_id].sRecoverable_fault.uiWhole_flag, BIT32_STRUCT, FAULT_BMS_CHECK_P2_ERROR);
			}
		}
 ...
 * @return
 */
void GB_BemTimeoutProcess(const GB_BEM_Data_t sBEM_data, const U8 ucGun_id)
{
	if (FALSE == GunIdValidCheck(ucGun_id))
	{
		return ;
	...
	{
		if (g_sGun_warn[ucGun_id].sRecoverable_warn.sItem.CCU_CRM_00_timeout != TRUE)
		{
			GunAlarmSet(&g_sGun_warn[ucGun_id].sRecoverable_warn.uiWhole_flag, BIT32_STRUCT, WARNING_CCU_CRM00_TIMEOUT);
			my_printf(USER_INFO, "GUNB: BEM error: CRM 00 is timeout!!\n");
		}
	}
	if ((U8)TRUE == sBEM_data.sTimeout.crm_AA)
	{
		if (g_sGun_warn[ucGun_id].sRecoverable_warn.sItem.CCU_CRM_AA_timeout != TRUE)
		{
			GunAlarmSet(&g_sGun_warn[ucGun_id].sRecoverable_warn.uiWhole_flag, BIT32_STRUCT, WARNING_CCU_CRMAA_TIMEOUT);
			my_printf(USER_INFO, "GUNB: BEM error: CRM AA is timeout!!\n");
		}
	}
	if ((U8)TRUE == sBEM_data.sTimeout.cml_cts)
	{
		if (g_sGun_warn[ucGun_id].sRecoverable_warn.sItem.CCU_CML_CTS_AA_timeout != TRUE)
		{
			GunAlarmSet(&g_sGun_warn[ucGun_id].sRecoverable_warn.uiWhole_flag, BIT32_STRUCT, WARNING_CCU_CML_CTS_TIMEOUT);
			my_printf(USER_INFO, "GUNB: BEM error: CML/CTS is timeout!!\n");
		}
	}
	if ((U8)TRUE == sBEM_data.sTimeout.cro)
	{
		if (g_sGun_warn[ucGun_id].sRecoverable_warn.sItem.CCU_CR0_timeout != TRUE)
		{
			GunAlarmSet(&g_sGun_warn[ucGun_id].sRecoverable_warn.uiWhole_flag, BIT32_STRUCT, WARNING_CCU_CRO_TIMEOUT);
			my_printf(USER_INFO, "GUNB: BEM error: CRO is timeout!!\n");
		}
	}
	if ((U8)TRUE == sBEM_data.sTimeout.ccs)
	{
		if (g_sGun_warn[ucGun_id].sRecoverable_warn.sItem.CCU_CCS_timeout != TRUE)
		{
			GunAlarmSet(&g_sGun_warn[ucGun_id].sRecoverable_warn.uiWhole_flag, BIT32_STRUCT, WARNING_CCU_CCS_TIMEOUT);
			my_printf(USER_INFO, "GUNB: BEM error: CCS is timeout!!\n");
		}
	}
	if ((U8)TRUE == sBEM_data.sTimeout.cst)
	{
		if (g_sGun_warn[ucGun_id].sRecoverable_warn.sItem.CCU_CST_timeout != TRUE)
		{
			GunAlarmSet(&g_sGun_warn[ucGun_id].sRecoverable_warn.uiWhole_flag, BIT32_STRUCT, WARNING_CCU_CST_TIMEOUT);
			my_printf(USER_INFO, "GUNB: BEM error: CST is timeout!!\n");
		}
	}
	if ((U8)TRUE == sBEM_data.sTimeout.csd)
	{
		if (g_sGun_warn[ucGun_id].sRecoverable_warn.sItem.CCU_CSD_timeout != TRUE)
		{
			GunAlarmSet(&g_sGun_warn[ucGun_id].sRecoverable_warn.uiWhole_flag, BIT32_STRUCT, WARNING_CCU_CSD_TIMEOUT);
