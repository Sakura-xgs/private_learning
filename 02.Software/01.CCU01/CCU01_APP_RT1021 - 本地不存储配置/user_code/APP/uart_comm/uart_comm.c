...
void my_printf(LogLevel level, const char *format, ...)
{
	...
	if (-1 != sockfd)
	{
		...
		g_sCMD_msg_control.sCCU_log_control.uiLog_len = u32MsgSize;
		(void)memcpy(g_sCMD_msg_control.sCCU_log_control.ucLog_data, u8MyDebugBuff, u32MsgSize);
		TcpSendControl(&g_sCMD_msg_control.sCCU_log_control.bSend_log_flag);
	}
#else
	u8MyDebugBuff[u32MsgSize] = '\0';
...
static void DebugControl(const U8 *ucBuf)
{
	...
	//if (DEBUG_MODE == g_sPile_data.ucPile_config_mode)
	{
		...
		case (U8)'d':
			...
			g_psHMI_data->ucGun_id = ((2U*g_sStorage_data.ucPile_num)-1U);
		...
		case (U8)'f':
			CHECK_PTR_NULL_NO_RETURN(g_psRFID_data);
			CHECK_PTR_NULL_NO_RETURN(g_psHMI_data);
			g_psHMI_data->ucGun_id = 2U*g_sStorage_data.ucPile_num;
			g_psHMI_data->ucCharge_control_type = RFID_START_MODE;
			//使能刷卡板记时
			g_psRFID_data->ucHmi_start_timing_flag = TIMER_OPEN_STATUS;
			//使能刷卡板寻卡
		...
		case (U8)'h':
			...
			g_psHMI_data->ucCharge_control_type = 0;
			g_psHMI_data->ucHmi_control_type = 0;
		...
		case 'D':
			...
			my_printf(USER_INFO, "close cig guna ASSIST relay\n");
			break;
		case 'E':
			CigContrlDataInput(CIG_CLOSE, CLOSE_GUNB_12V_ASSIST);
			my_printf(USER_INFO, "close cig gunb ASSIST relay\n");
			break;
		case 'F':
...
static void DebugRead(const U8 *ucBuf)
{
	...
	//if (DEBUG_MODE == g_sPile_data.ucPile_config_mode)
	{
		...
		case (U8)'1':
			...
			break;
		case (U8)'2':
			my_printf(USER_INFO, "GUN_A id = %d status = %d GUN_B id = %d status = %d\n", g_sStorage_data.sPublic_data[GUN_A].ucGun_id, g_sGun_data[GUN_A].ucGun_common_status,
					g_sStorage_data.sPublic_data[GUN_B].ucGun_id, g_sGun_data[GUN_B].ucGun_common_status);
			break;
		case (U8)'3':
			U8 ucTemp[6] = {0};
			U8 ucData[18] = {0};
		...
		case (U8)'9':
			...
			my_printf(USER_INFO, "time = %d/%d/%d/%d:%d:%d\n", year, month, day, hour+8U, minute, second);
			break;
		case (U8)'a':
