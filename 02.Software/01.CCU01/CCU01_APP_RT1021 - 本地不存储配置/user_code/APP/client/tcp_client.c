...
static SystemSemaphores_t g_sSystemSemaphores =
{
    .tcp_send_mutex = NULL,
    .alarm_mutex = NULL,
    .client_upload_log_sem = NULL
};
static LocalState_t g_sLocalState =
{
    .unMessageId = 1,
    .ucAlarmFlag = 0,
    .psMsgHead = NULL
};
__BSS(SRAM_OC) static CommBuffs_t g_sCommBuffs = {0};
__BSS(SRAM_OC) static Alarm_Area_Set_t g_sAlarm_area_set = {0};
__BSS(SRAM_OC) static Gun_Alarm_Upload_Buff_t g_sGun_alarm_upload_buff[2] = {0};
__BSS(SRAM_OC) static Pile_Alarm_Upload_Buff_t g_sPile_alarm_upload_buff = {0};
__BSS(SRAM_OC) static Device_Msg_Control_t g_sModule_msg_control = {0};

__BSS(SRAM_OC) Gun_ID_Status_t g_sGun_id_status = {0};
__BSS(SRAM_OC) Connect_Control_t g_sConnect_control = {0};
__BSS(SRAM_OC) Storage_Gun_Data_t g_sStorage_data = {0};
__BSS(SRAM_OC) Gun_Data_t g_sGun_data[2] = {0};
__BSS(SRAM_OC) Pile_Data_t g_sPile_data = {0};
__BSS(SRAM_OC) CMD_Msg_Control_t g_sCMD_msg_control = {0};

SemaphoreHandle_t g_sAlarm_tigger_sem = NULL;
S32 sockfd = -1;
BOOL g_bReceive_server_config = FALSE;
...
BOOL CheckConfigMsg(void)
{
	...
	//A枪id
	if (TRUE == CheckConfig(CCU_SET_SIG_ID_GUN_ID_A, g_sStorage_data.sPublic_data[GUN_A].ucGun_id))
	{
		return TRUE;
	}
	//电表型号
	...
	//B枪id
	if (TRUE == CheckConfig(CCU_SET_SIG_ID_GUN_ID_B, g_sStorage_data.sPublic_data[GUN_B].ucGun_id))
	{
		return TRUE;
	}
	//电表型号
...
void CheckServerConfig(void)
{
	while (1)
	{
		if (TRUE == g_bReceive_server_config)
		{
			break;
		}
		vTaskDelay(500);
...
{
	//防止传输中close导致错误
	if (pdTRUE == xSemaphoreTake(g_sSystemSemaphores.tcp_send_mutex, portMAX_DELAY))
	{
		if (-1 != sockfd)
		{
		...
		(void)xSemaphoreGive(g_sSystemSemaphores.tcp_send_mutex);
	}
}
...
BOOL CheckChargingStatus(const U8 ucGun_id)
{
	...
	if (((U8)STA_CHARGING != g_sGun_data[ucGun_id].ucGun_common_status)
			&& ((U8)STA_START_CHARGE != g_sGun_data[ucGun_id].ucGun_common_status))
	{
		return FALSE;
	}
	else
...
void SpecialStatusSetProcess(const U8 ucGun_id, const U8 ucStatus)
{
	...
	g_sGun_data[ucGun_id].ucGun_special_status = ucStatus;//通知充电逻辑部分

	if (g_sGun_data[ucGun_id].ucGun_common_status != (U8)STA_FAULT)
	{
		GunStatusChange(ucGun_id, STA_UNAVAILABLE, FALSE, FALSE);
	}
}
...
void TcpSendControl(U8* ucFlag)
{
	*ucFlag = (U8)TRUE;
	...
	if (NULL != g_sSystemSemaphores.client_upload_log_sem)
	{
		(void)xSemaphoreGive(g_sSystemSemaphores.client_upload_log_sem);
	}
}
...
U8 SetAlarmFlagFunc(void)
{
	...
	g_sAlarm_area_set.pile_warn_flag = (U8)iTmp;

	if (0U != g_sGun_warn[GUN_A].sRecoverable_warn.uiWhole_flag)
	{
		g_sAlarm_area_set.gun_warn_flag_A = (U8)TRUE;
	}
	else
	{
		g_sAlarm_area_set.gun_warn_flag_A = (U8)FALSE;
	}
	if (0U != g_sGun_warn[GUN_B].sRecoverable_warn.uiWhole_flag)
	{
		g_sAlarm_area_set.gun_warn_flag_B = (U8)TRUE;
	}
	else
	{
		g_sAlarm_area_set.gun_warn_flag_B = (U8)FALSE;
	}

	//故障
	(void)GetSigVal(ALARM_ID_PILE_ERROR_LEV_2, &iTmp);
	g_sAlarm_area_set.pile_error_flag = (U8)iTmp;
	if (0L != iTmp)
	{
		g_sAlarm_area_set.gun_error_flag_A = (U8)TRUE;
		g_sAlarm_area_set.gun_error_flag_B = (U8)TRUE;
	}
	else
	{
		...
		{
			g_sAlarm_area_set.gun_error_flag_A = (U8)FALSE;
		}
		...
		{
			g_sAlarm_area_set.gun_error_flag_B = (U8)FALSE;
		}
	}

	(void)memcpy(&ucTmp, &g_sAlarm_area_set, 1);
...
//	g_sStorage_data.sPublic_data[GUN_A].ucGun_id = ((2U*g_sStorage_data.ucPile_num)-1U);
...
//	g_sStorage_data.sPublic_data[GUN_B].ucGun_id = 2U*g_sStorage_data.ucPile_num;
...
/**
 * @brief 封装tcp发送
 * @param g_sCommBuffs.ucSendBuff：发送数据
 * @param g_sCommBuffs.uiSendLength：发送长度
 * @return
 */
BOOL TcpSend(const U8 *ucSendBuff, U32 uiSendLength)
{
	BOOL bflag = 0;
	S32 sTemp = (S32)uiSendLength;

	if (-1 != sockfd)
	{
		if (pdTRUE == xSemaphoreTake(g_sSystemSemaphores.tcp_send_mutex, portMAX_DELAY))
		{
			if (sTemp == send(sockfd, ucSendBuff, uiSendLength, 0))
			{
				bflag = TRUE;
			}
			else
...
static U32 CCU_Data0001(U8 *pucDst)
{
	...
	if (g_sLocalState.unMessageId >= 65000U)
	{
		g_sLocalState.unMessageId = 1;
	}
	pucDst[i] = (U8)((g_sLocalState.unMessageId >> 8U) & 0xffU);
	i++;
	pucDst[i] = (U8)(g_sLocalState.unMessageId & 0xffU);
	i++;
	g_sLocalState.unMessageId++;
	...
	i++;
	//A枪id
	pucDst[i] = g_sStorage_data.sPublic_data[GUN_A].ucGun_id;
	i++;
	//最大输出电压
	...
	i++;
	//A枪告警枪温
	nTemp = g_sStorage_data.sPublic_data[GUN_A].unGun_warn_temp+60;
	pucDst[i] = (U8)(((U16)nTemp >> 8) & 0xffU);
	i++;
	pucDst[i] = (U8)((U16)nTemp & 0xffU);
	i++;
	//A枪故障枪温
	nTemp = g_sStorage_data.sPublic_data[GUN_A].unGun_fault_temp+60;
	pucDst[i] = (U8)(((U16)nTemp >> 8) & 0xffU);
	i++;
	pucDst[i] = (U8)((U16)nTemp & 0xffU);
	i++;
	//A枪正负极枪温温差阈值
	nTemp = g_sStorage_data.sPublic_data[GUN_A].unGun_diff_temp+60;
	pucDst[i] = (U8)(((U16)nTemp >> 8) & 0xffU);
	i++;
	pucDst[i] = (U8)((U16)nTemp & 0xffU);
	i++;
	...
	i++;
	//B枪id
	pucDst[i] = g_sStorage_data.sPublic_data[GUN_B].ucGun_id;
	i++;
	//最大输出电压
	...
	i++;
	//B枪告警枪温
	nTemp = g_sStorage_data.sPublic_data[GUN_B].unGun_warn_temp+60;
	pucDst[i] = (U8)(((U16)nTemp >> 8) & 0xffU);
	i++;
	pucDst[i] = (U8)((U16)nTemp & 0xffU);
	i++;
	//B枪故障枪温
	nTemp = g_sStorage_data.sPublic_data[GUN_B].unGun_fault_temp+60;
	pucDst[i] = (U8)(((U16)nTemp >> 8) & 0xffU);
	i++;
	pucDst[i] = (U8)((U16)nTemp & 0xffU);
	i++;
	//B枪正负极枪温温差阈值
	nTemp = g_sStorage_data.sPublic_data[GUN_B].unGun_diff_temp+60;
	pucDst[i] = (U8)(((U16)nTemp >> 8) & 0xffU);
	i++;
	pucDst[i] = (U8)((U16)nTemp & 0xffU);
	i++;
...
{
	(void)memset(g_sCommBuffs.ucSendBuff, 0, sizeof(g_sCommBuffs.ucSendBuff));

	g_sCommBuffs.uiSendLength = CCU_Data0001(g_sCommBuffs.ucSendBuff);

	//MessageDebug(g_sCommBuffs.ucSendBuff, g_sCommBuffs.uiSendLength);

	if (TRUE == TcpSend(g_sCommBuffs.ucSendBuff, g_sCommBuffs.uiSendLength))
	{
		//发送成功
...
static U32 CCU_Data0002(U8 *pucDst)
{
	...
	pucDst[i] = (U8)((g_sLocalState.unMessageId >> 8) & 0xffU);
	i++;
	pucDst[i] = (U8)(g_sLocalState.unMessageId & 0xffU);
	i++;
	g_sLocalState.unMessageId++;
	...
	//故障/告警所属区域,bit位表示
	pucDst[i] = g_sLocalState.ucAlarmFlag;
	i++;

	if (NULL != g_sSystemSemaphores.alarm_mutex)
	{
		if (pdTRUE == xSemaphoreTake(g_sSystemSemaphores.alarm_mutex, portMAX_DELAY))
		{
			//桩告警
			(void)memcpy(&pucDst[i], &g_sPile_alarm_upload_buff.sPile_warn, sizeof(g_sPile_alarm_upload_buff.sPile_warn));
			i+=PILE_DEVICE_LEN;
			//桩故障
			(void)memcpy(&pucDst[i], &g_sPile_alarm_upload_buff.sPile_fault, sizeof(g_sPile_alarm_upload_buff.sPile_fault));
			i+=PILE_DEVICE_LEN;
			(void)xSemaphoreGive(g_sSystemSemaphores.alarm_mutex);
			//uPRINTF("RFID status = %d\n", g_sPile_alarm_upload_buff.sPile_warn.ucPile_warn_buf[0]);
		}
	}
	//预留
	...
	//枪普通状态
	//pucDst[i] = g_sGun_data[GUN_A].ucGun_common_status;
	if (0U != g_sAlarm_area_set.gun_error_flag_A)
	{
		//充电中不直接置位为故障
	...
	{
		//恢复
		if ((U8)STA_FAULT == g_sGun_data[GUN_A].ucGun_common_status)
		{
	...
	i++;
	//cp/cc1状态
	pucDst[i] = g_sGun_data[GUN_A].ucConnect_status;
	i++;
	//DC+温度
	nTemp = g_sGun_data[GUN_A].nDC_positive_temp+60;
	pucDst[i] = (U8)(((U16)nTemp >> 8) & 0xffU);
	i++;
	pucDst[i] = (U8)((U16)nTemp & 0xffU);
	i++;
	//DC-温度
	nTemp = g_sGun_data[GUN_A].nDC_negative_temp+60;
	pucDst[i] = (U8)(((U16)nTemp >> 8) & 0xffU);
	i++;
	pucDst[i] = (U8)((U16)nTemp & 0xffU);
	i++;
	...
	if (NULL != g_sSystemSemaphores.alarm_mutex)
	{
		...
		{
			//枪告警
			(void)memcpy(&pucDst[i], &g_sGun_alarm_upload_buff[GUN_A].sGun_warn, sizeof(g_sGun_alarm_upload_buff[GUN_A].sGun_warn));
			i+=GUN_WARN_DEVICE_LEN;
			//预留
			i+=GUN_WARN_RESERVE_LEN;
			//枪故障
			(void)memcpy(&pucDst[i], &g_sGun_alarm_upload_buff[GUN_A].sGun_fault, sizeof(g_sGun_alarm_upload_buff[GUN_A].sGun_fault));
	...
	//枪普通状态
	//pucDst[i] = g_sGun_data[GUN_B].ucGun_common_status;
	//有故障
	if (0U != g_sAlarm_area_set.gun_error_flag_B)
	{
		//PCU要求充电中不直接置位为故障
	...
	{
		//恢复
		if ((U8)STA_FAULT == g_sGun_data[GUN_B].ucGun_common_status)
		{
	...
	i++;
	//cp/cc1状态
	pucDst[i] = g_sGun_data[GUN_B].ucConnect_status;
	i++;
	//DC+温度
	nTemp = g_sGun_data[GUN_B].nDC_positive_temp+60;
	pucDst[i] = (U8)(((U16)nTemp >> 8) & 0xffU);
	i++;
	pucDst[i] = (U8)((U16)nTemp & 0xffU);
	i++;
	//DC-温度
	nTemp = g_sGun_data[GUN_B].nDC_negative_temp+60;
	pucDst[i] = (U8)(((U16)nTemp >> 8) & 0xffU);
	i++;
	pucDst[i] = (U8)((U16)nTemp & 0xffU);
	i++;
	...
	{
		//枪告警
		(void)memcpy(&pucDst[i], &g_sGun_alarm_upload_buff[GUN_B].sGun_warn, sizeof(g_sGun_alarm_upload_buff[GUN_B].sGun_warn));
		i+=GUN_WARN_DEVICE_LEN;
		//预留
		i+=GUN_WARN_RESERVE_LEN;
		//枪故障
		(void)memcpy(&pucDst[i], &g_sGun_alarm_upload_buff[GUN_B].sGun_fault, sizeof(g_sGun_alarm_upload_buff[GUN_B].sGun_fault));
		i+=GUN_ERROR_DEVICE_LEN;
		//预留
		i+=GUN_ERROR_RESERVE_LEN;
		(void)xSemaphoreGive(g_sSystemSemaphores.alarm_mutex);
...
{
	(void)memset(g_sCommBuffs.ucSendBuff, 0, sizeof(g_sCommBuffs.ucSendBuff));
	g_sCommBuffs.uiSendLength = CCU_Data0002(g_sCommBuffs.ucSendBuff);

	MessageDebug(g_sCommBuffs.ucSendBuff, g_sCommBuffs.uiSendLength);

	(void)TcpSend(g_sCommBuffs.ucSendBuff, g_sCommBuffs.uiSendLength);
}
...
static U32 CCU_Data0003(U8 *pucDst)
{
	...
	{
		if (g_sGun_id_status.ucStart_control_id == g_sStorage_data.sPublic_data[GUN_A].ucGun_id)
		{
			//枪id
			pucDst[i] = g_sStorage_data.sPublic_data[GUN_A].ucGun_id;
			i++;
			...
			i++;
			//启动类型
			pucDst[i] = g_sGun_data[GUN_A].sTemp.ucStart_Charge_type;
			i++;
			//idtag
	...
	{
		if (g_sGun_id_status.ucFinish_control_id == g_sStorage_data.sPublic_data[GUN_A].ucGun_id)
		{
			//枪id
			pucDst[i] = g_sStorage_data.sPublic_data[GUN_A].ucGun_id;
			i++;
			...
			i++;
			//结束类型
			pucDst[i] = g_sGun_data[GUN_A].sTemp.ucStop_Charge_type;
			i++;
			//idtag
			(void)memcpy(&pucDst[i], g_sGun_data[GUN_A].sTemp.ucIdtag, sizeof(g_sGun_data[GUN_A].sTemp.ucIdtag));
			i += sizeof(g_sGun_data[GUN_A].sTemp.ucIdtag);
	...
	{
		if (g_sGun_id_status.ucStart_control_id == g_sStorage_data.sPublic_data[GUN_B].ucGun_id)
		{
			//枪id
			pucDst[i] = g_sStorage_data.sPublic_data[GUN_B].ucGun_id;
			i++;
			...
			i++;
			//启动类型
			pucDst[i] = g_sGun_data[GUN_B].sTemp.ucStart_Charge_type;
			i++;
			//idtag
	...
	{
		if (g_sGun_id_status.ucFinish_control_id == g_sStorage_data.sPublic_data[GUN_B].ucGun_id)
		{
			//枪id
			pucDst[i] = g_sStorage_data.sPublic_data[GUN_B].ucGun_id;
			i++;
			...
			i++;
			//结束类型
			pucDst[i] = g_sGun_data[GUN_B].sTemp.ucStop_Charge_type;
			i++;
			//idtag
			(void)memcpy(&pucDst[i], g_sGun_data[GUN_B].sTemp.ucIdtag, sizeof(g_sGun_data[GUN_B].sTemp.ucIdtag));
			i += sizeof(g_sGun_data[GUN_B].sTemp.ucIdtag);
...
static void CCU_Send0003(void)
{
	...
	g_sCommBuffs.uiSendLength = CCU_Data0003(g_sCommBuffs.ucSendBuff);
...
static U32 CCU_Data0004(U8 *pucDst)
{
	...
	i++;
	//运行状态
	pucDst[i] = g_sGun_data[GUN_A].sTemp.ucCharge_current_step;
	i++;
	//最高允许电压(同绝缘检测电压)
	...
	i++;
	//运行状态
	pucDst[i] = g_sGun_data[GUN_B].sTemp.ucCharge_current_step;
	i++;
	//最高允许电压
...
static void CCU_Send0004(void)
{
	...
	g_sCommBuffs.uiSendLength = CCU_Data0004(g_sCommBuffs.ucSendBuff);
...
static void CCU_Send0005(void)
{
	...
	g_sCommBuffs.uiSendLength = CCU_Data0005(g_sCommBuffs.ucSendBuff);
...
{
	(void)memset(g_sCommBuffs.ucSendBuff, 0, sizeof(g_sCommBuffs.ucSendBuff));
	g_sCommBuffs.uiSendLength = CCU_Data0006(g_sCommBuffs.ucSendBuff);
...
static void CCU_Send0007(void)
{
	...
	g_sCommBuffs.uiSendLength = CCU_Data0007(g_sCommBuffs.ucSendBuff);
...
static void CCU_Send0008(void)
{
	...
	g_sCommBuffs.uiSendLength = CCU_Data0008(g_sCommBuffs.ucSendBuff);
...
static void CCU_Send0009(void)
{
	...
	g_sCommBuffs.uiSendLength = CCU_Data0009(g_sCommBuffs.ucSendBuff);
...
static void CCU_Send0010(void)
{
	...
	g_sCommBuffs.uiSendLength = CCU_Data0010(g_sCommBuffs.ucSendBuff);
...
static U32 CCU_Data0011(U8 *pucDst)
{
	...
	else if (g_sGun_id_status.ucFinish_control_id == g_sStorage_data.sPublic_data[GUN_B].ucGun_id)
	{
		//枪id
		pucDst[i] = g_sStorage_data.sPublic_data[GUN_B].ucGun_id;
		i++;
	...
	{
		my_printf(USER_ERROR, "%s:%d ucFinish_control_id error = %d\n", __FILE__, __LINE__, g_sGun_id_status.ucFinish_control_id);
	}
...
static void CCU_Send0011(void)
{
	...
	g_sCommBuffs.uiSendLength = CCU_Data0011(g_sCommBuffs.ucSendBuff);
...
static U32 CCU_Data0014(U8 *pucDst)
{
	...
	CHECK_MSG_LEN(SEND_BUFF_SIZE, g_sCMD_msg_control.sCCU_log_control.uiLog_len + MSGBODY_OFFSET);
	...
	pucDst[i] = (U8)((g_sCMD_msg_control.sCCU_log_control.uiLog_len >> 8) & 0xffU);
	i++;
	pucDst[i] = (U8)(g_sCMD_msg_control.sCCU_log_control.uiLog_len & 0xffU);
	i++;

	(void)memcpy(&pucDst[i], g_sCMD_msg_control.sCCU_log_control.ucLog_data, g_sCMD_msg_control.sCCU_log_control.uiLog_len);
	i+=g_sCMD_msg_control.sCCU_log_control.uiLog_len;

	if (i == (g_sCMD_msg_control.sCCU_log_control.uiLog_len + MSGBODY_OFFSET))
	{
		return i;
	}
...
{
	(void)memset(g_sCommBuffs.ucSendBuff, 0, sizeof(g_sCommBuffs.ucSendBuff));
	g_sCommBuffs.uiSendLength = CCU_Data0014(g_sCommBuffs.ucSendBuff);
...
//
//	if (g_sLocalState.unMessageId >= 65000)
//	{
//		g_sLocalState.unMessageId = 1;
//	}
//	pucDst[i] = (U8)((g_sLocalState.unMessageId >> 8) & 0xffU);
...
//	(void)memset(g_sCommBuffs.ucSendBuff, 0, sizeof(g_sCommBuffs.ucSendBuff));
//
//	g_sCommBuffs.uiSendLength = CCU_Data0013(g_sCommBuffs.ucSendBuff);
//
//	MessageDebug(g_sCommBuffs.ucSendBuff, g_sCommBuffs.uiSendLength);
//
//	if (-1 != sockfd)
//	{
//		if (g_sCommBuffs.uiSendLength == send(sockfd, g_sCommBuffs.ucSendBuff, g_sCommBuffs.uiSendLength, 0))
//		{
//			//发送成功
//		}
//		else
...
static void CCU_Send0016(void)
{
	...
	g_sCommBuffs.uiSendLength = CCU_Data0016(g_sCommBuffs.ucSendBuff);
...
static void AnalysisMsgbody8001(U16 unMsg_len, const U8 *pcSrc)
{
	...
	ucTimestamp = BigEndianToLittleEndian(ucTimestamp);
	//同步rtc
...
static void AnalysisMsgbody8003(U16 unMsg_len, const U8 *pcSrc)
{
	...
	if (pcSrc[0] == g_sStorage_data.sPublic_data[GUN_A].ucGun_id)
	{
		//鉴权成功
		if (TRUE == pcSrc[1])
		{
			...
			{
				g_sGun_id_status.ucFinish_control_id = pcSrc[0];
				//告知HMI
				if (NULL != g_psHMI_data)
				{
					g_psHMI_data->ucAuthorize_flag[GUN_A] = AUTHORIZE_SUCCESS;
		...
		g_sGun_id_status.ucStart_control_id = 0;
	}
	else if (pcSrc[0] == g_sStorage_data.sPublic_data[GUN_B].ucGun_id)
	{
		//鉴权成功
		if (TRUE == pcSrc[1])
		{
...
//		g_sGun_id_status.ucFinish_control_id = 0;
//	}
//	else if (pcSrc[0] == g_sStorage_data.sPublic_data[GUN_B].ucGun_id)
//	{
...
static U32 CCU_Data8020(U8 *pucDst, BOOL bFlag)
{
	...
	(void)memcpy(&pucDst[i], &g_sLocalState.psMsgHead->unMsg_id, sizeof(g_sLocalState.psMsgHead->unMsg_id));
	i+=sizeof(g_sLocalState.psMsgHead->unMsg_id);
...
{
	(void)memset(g_sCommBuffs.ucResponseBuff, 0, sizeof(g_sCommBuffs.ucResponseBuff));

	g_sCommBuffs.uiResponseLength = CCU_Data8020(g_sCommBuffs.ucResponseBuff, bFlag);

	MessageDebug(g_sCommBuffs.ucResponseBuff, g_sCommBuffs.uiResponseLength);

	(void)TcpSend(g_sCommBuffs.ucResponseBuff, g_sCommBuffs.uiResponseLength);
}
...
static void AnalysisMsgbody0020(U16 unMsg_len, const U8 *pcSrc)
{
	...
	if (sParse_0020.ucGun_id == g_sStorage_data.sPublic_data[GUN_A].ucGun_id)
	{
		if ((U8)STA_PLUGING == g_sGun_data[GUN_A].ucGun_common_status)
		{
			//if (1)
			if (sParse_0020.ucAuth_status == (U8)AUTH_SUCCESS)
			{
				//记录启动数据
				g_sGun_data[GUN_A].sTemp.uiStart_time = xTaskGetTickCount();
				CHECK_PTR_NULL_NO_RETURN(g_psMeter_data);
				g_sGun_data[GUN_A].sTemp.uiStart_meter_dn = g_sGun_data[GUN_A].uiCurrent_meter_dn;
				//先于实时数据发送前更新状态
				GunStatusChange(GUN_A, STA_START_CHARGE, FALSE, FALSE);
				//确保心跳状态变更报文早于响应报文上传
				vTaskDelay(100);
				//回复启动成功
				CCU_Send8020(TRUE);
				//设置启动枪id
				g_sGun_id_status.ucStart_control_id = sParse_0020.ucGun_id;
				(void)memcpy(&g_sGun_data[GUN_A].sTemp.uiOrder_id, &sParse_0020.ucOrder[0], sizeof(g_sGun_data[GUN_A].sTemp.uiOrder_id));
				g_sGun_data[GUN_A].sTemp.ucStart_Charge_type = sParse_0020.ucStart_type;
				my_printf(USER_INFO, "GUN_A uiOrder_id = %u charge_type = %d\n", g_sGun_data[GUN_A].sTemp.uiOrder_id,
						g_sGun_data[GUN_A].sTemp.ucStart_Charge_type);

				//启动充电
				StartChargeControl(GUN_A);
				//上传充电实时数据
				TcpSendControl(&g_sCMD_msg_control.sCharging_control.bSend_charging_data_flag);
				//使能充电实时数据定时器
				//TimerSingleStart(CHARGING_TIMER);
				g_sCMD_msg_control.sCharging_control.bCharging_timeup_flag = TRUE;
			}
			else
			{
				my_printf(USER_INFO, "GUN_A start auth failed\n");
			}
		}
		else
		{
			//回复启动失败
			CCU_Send8020(FALSE);
			my_printf(USER_INFO, "GUN_A The current status does't support start charging:common_status = %d\n",
					g_sGun_data[GUN_A].ucGun_common_status);
		}
	}
	else if (sParse_0020.ucGun_id == g_sStorage_data.sPublic_data[GUN_B].ucGun_id)
	{
		if ((U8)STA_PLUGING == g_sGun_data[GUN_B].ucGun_common_status)
		{
			//if (1)
			if (sParse_0020.ucAuth_status == (U8)AUTH_SUCCESS)
			{
				//记录启动数据
				g_sGun_data[GUN_B].sTemp.uiStart_time = xTaskGetTickCount();
				CHECK_PTR_NULL_NO_RETURN(g_psMeter_data);
				g_sGun_data[GUN_B].sTemp.uiStart_meter_dn = g_sGun_data[GUN_B].uiCurrent_meter_dn;
				//先于实时数据发送前更新状态
				GunStatusChange(GUN_B, STA_START_CHARGE, FALSE, FALSE);
				//确保状态变更报文早于响应报文上传
				vTaskDelay(100);
				//回复启动成功
				CCU_Send8020(TRUE);
				//设置启动枪id
				g_sGun_id_status.ucStart_control_id = sParse_0020.ucGun_id;
				(void)memcpy(&g_sGun_data[GUN_B].sTemp.uiOrder_id, &sParse_0020.ucOrder[0], sizeof(g_sGun_data[GUN_B].sTemp.uiOrder_id));
				g_sGun_data[GUN_B].sTemp.ucStart_Charge_type = sParse_0020.ucStart_type;
				my_printf(USER_INFO, "GUN_B uiOrder_id = %u charge_type = %d\n", g_sGun_data[GUN_B].sTemp.uiOrder_id,
						g_sGun_data[GUN_B].sTemp.ucStart_Charge_type);

				//启动充电
				StartChargeControl(GUN_B);
				//上传充电实时数据
				TcpSendControl(&g_sCMD_msg_control.sCharging_control.bSend_charging_data_flag);
				//使能充电实时数据定时器
				//TimerSingleStart(CHARGING_TIMER);
			...
			{
				my_printf(USER_INFO, "GUN_B start auth failed\n");
			}
		}
		else
		{
			//回复启动失败
			CCU_Send8020(FALSE);
			my_printf(USER_INFO, "GUN_B The current status does't support start charging:common_status = %d\n",
					g_sGun_data[GUN_B].ucGun_common_status);
		}
	}
	else
	{
		//回复启动失败
		CCU_Send8020(FALSE);
		my_printf(USER_INFO, "receive 0020 server start charge id error: %d GUN_A=%d GUN_B=%d\n", pcSrc[0],g_sStorage_data.sPublic_data[GUN_A].ucGun_id,
				g_sStorage_data.sPublic_data[GUN_B].ucGun_id);
	}
}
...
static void CCU_Send8021(BOOL bFlag)
{
	...
	g_sCommBuffs.uiResponseLength = CCU_Data8021(g_sCommBuffs.ucResponseBuff, bFlag);
...
static void AnalysisMsgbody0021(U16 unMsg_len, const U8 *pcSrc)
{
	...
	{
		//进入充电中才允许主动停充
		if ((U8)STA_CHARGING == g_sGun_data[GUN_A].ucGun_common_status)
		{
			g_sGun_id_status.ucFinish_control_id = pcSrc[0];
			g_sGun_data[GUN_A].sTemp.ucStop_Charge_type = pcSrc[1];
			//停止充电
			StopChargeControl(GUN_A);
			CCU_Send8021(TRUE);
		}
		...
		{
			CCU_Send8021(FALSE);
			my_printf(USER_ERROR, "%s:%d The current state does not allow stopping charging = %d\n", __FILE__, __LINE__, g_sGun_data[GUN_A].ucGun_common_status);
		}
	}
	else if (pcSrc[0] == g_sStorage_data.sPublic_data[GUN_B].ucGun_id)
	{
		//进入充电中才允许主动停充
		if ((U8)STA_CHARGING == g_sGun_data[GUN_B].ucGun_common_status)
		{
			g_sGun_id_status.ucFinish_control_id = pcSrc[0];
			g_sGun_data[GUN_B].sTemp.ucStop_Charge_type = pcSrc[1];
			//停止充电
			StopChargeControl(GUN_B);
			CCU_Send8021(TRUE);
		}
		...
		{
			CCU_Send8021(FALSE);
			my_printf(USER_ERROR, "%s:%d The current state does not allow stopping charging = %d\n", __FILE__, __LINE__, g_sGun_data[GUN_B].ucGun_common_status);
		}
	}
	else
	{
...
static void CCU_Send8022(BOOL bFlag)
{
	...
	g_sCommBuffs.uiResponseLength = CCU_Data8022(g_sCommBuffs.ucResponseBuff, bFlag);
...
static void AnalysisMsgbody0022(U16 unMsg_len, const U8 *pcSrc)
{
	...
	if (pcSrc[0] == g_sStorage_data.sPublic_data[GUN_A].ucGun_id)
	{
		if (START_BOOK == pcSrc[1])
		{
			if ((U8)STA_IDLE == g_sGun_data[GUN_A].ucGun_common_status)
			{
				//兼容OCPP转换层逻辑，响应报文先于状态变更报文发送
				CCU_Send8022(TRUE);
				GunStatusChange(GUN_A, STA_BOOK, FALSE, FALSE);
				my_printf(USER_INFO, "GUN_A receive server control:start book\n");
			}
		}
		else if (CANCEL_BOOK == pcSrc[1])
		{
			if ((U8)STA_BOOK == g_sGun_data[GUN_A].ucGun_common_status)
			{
				CCU_Send8022(TRUE);
				GunStatusChange(GUN_A, STA_IDLE, FALSE, TRUE);
				my_printf(USER_INFO, "GUN_A receive server control:cancel book\n");
			}
		}
		else
		{
			CCU_Send8022(FALSE);
			my_printf(USER_ERROR, "%s:%d GUN_A server control book type error\n", __FILE__, __LINE__);
		}
	}
	else if (pcSrc[0] == g_sStorage_data.sPublic_data[GUN_B].ucGun_id)
	{
		if (START_BOOK == pcSrc[1])
		{
			if ((U8)STA_IDLE == g_sGun_data[GUN_B].ucGun_common_status)
			{
				CCU_Send8022(TRUE);
				GunStatusChange(GUN_B, STA_BOOK, FALSE, FALSE);
				my_printf(USER_INFO, "GUN_B receive server control:start book\n");
			}
		}
		else if (CANCEL_BOOK == pcSrc[1])
		{
			if ((U8)STA_BOOK == g_sGun_data[GUN_B].ucGun_common_status)
			{
				CCU_Send8022(TRUE);
				GunStatusChange(GUN_B, STA_IDLE, FALSE, TRUE);
				my_printf(USER_INFO, "GUN_B receive server control:cancel book\n");
			}
		}
		else
		{
			CCU_Send8022(FALSE);
			my_printf(USER_ERROR, "%s:%d GUN_B server control book type error\n", __FILE__, __LINE__);
		}
	}
	else
	{
...
static void CCU_Send8024(void)
{
	...
	g_sCommBuffs.uiResponseLength = CCU_Data8024(g_sCommBuffs.ucResponseBuff);
...
static void CCU_Send8025(BOOL bFlag)
{
	...
	g_sCommBuffs.uiResponseLength = CCU_Data8025(g_sCommBuffs.ucResponseBuff, bFlag);
	...
	if (sGun_control_parse.bReboot_flag)//重启
	{
		...
		else
		{
			my_printf(USER_INFO, "The current state does't allow restarting GUN_A:common_status = %d GUN_B common_status = %d\n",
					g_sGun_data[GUN_A].ucGun_common_status, g_sGun_data[GUN_A].ucGun_common_status);
		}
	}
...
static void CCU_Send8026(BOOL bFlag)
{
	...
	g_sCommBuffs.uiResponseLength = CCU_Data8026(g_sCommBuffs.ucResponseBuff, bFlag);
...
//
	...
	vTaskDelay(3);
	(void)SetSigVal(CCU_SET_SIG_ID_GUN_ID_A, (S32)g_sStorage_data.sPublic_data[GUN_A].ucGun_id);
	vTaskDelay(3);
	...
	//枪温配置
	g_sStorage_data.sPublic_data[GUN_A].unGun_warn_temp = ((((g_sStorage_data.sPublic_data[GUN_A].unGun_warn_temp & 0xFF00U) >> 8) | ((g_sStorage_data.sPublic_data[GUN_A].unGun_warn_temp & 0xFFU) << 8)) - 60);
	(void)SetSigVal(CCU_SET_SIG_ID_WARN_TEMP_A, (S32)g_sStorage_data.sPublic_data[GUN_A].unGun_warn_temp);
	vTaskDelay(3);
	g_sStorage_data.sPublic_data[GUN_A].unGun_fault_temp = ((((g_sStorage_data.sPublic_data[GUN_A].unGun_fault_temp & 0xFF00U) >> 8) | ((g_sStorage_data.sPublic_data[GUN_A].unGun_fault_temp & 0xFFU) << 8)) - 60);
	(void)SetSigVal(CCU_SET_SIG_ID_FAULT_TEMP_A, (S32)g_sStorage_data.sPublic_data[GUN_A].unGun_fault_temp);
	vTaskDelay(3);
	g_sStorage_data.sPublic_data[GUN_A].unGun_diff_temp = ((((g_sStorage_data.sPublic_data[GUN_A].unGun_diff_temp & 0xFF00U) >> 8) | ((g_sStorage_data.sPublic_data[GUN_A].unGun_diff_temp & 0xFFU) << 8)) - 60);
	...
	vTaskDelay(3);
	(void)SetSigVal(CCU_SET_SIG_ID_GUN_ID_B, (S32)g_sStorage_data.sPublic_data[GUN_B].ucGun_id);
	vTaskDelay(3);
	...
	//枪温配置
	g_sStorage_data.sPublic_data[GUN_B].unGun_warn_temp = ((((g_sStorage_data.sPublic_data[GUN_B].unGun_warn_temp & 0xFF00U) >> 8) | ((g_sStorage_data.sPublic_data[GUN_B].unGun_warn_temp & 0xFFU) << 8)) - 60);
	(void)SetSigVal(CCU_SET_SIG_ID_WARN_TEMP_B, (S32)g_sStorage_data.sPublic_data[GUN_B].unGun_warn_temp);
	vTaskDelay(3);
	g_sStorage_data.sPublic_data[GUN_B].unGun_fault_temp = ((((g_sStorage_data.sPublic_data[GUN_B].unGun_fault_temp & 0xFF00U) >> 8) | ((g_sStorage_data.sPublic_data[GUN_B].unGun_fault_temp & 0xFFU) << 8)) - 60);
	(void)SetSigVal(CCU_SET_SIG_ID_FAULT_TEMP_B, (S32)g_sStorage_data.sPublic_data[GUN_B].unGun_fault_temp);
	vTaskDelay(3);
	g_sStorage_data.sPublic_data[GUN_B].unGun_diff_temp = ((((g_sStorage_data.sPublic_data[GUN_B].unGun_diff_temp & 0xFF00U) >> 8) | ((g_sStorage_data.sPublic_data[GUN_B].unGun_diff_temp & 0xFFU) << 8)) - 60);
...
//	i++;
//	//DC+温度
//	pucDst[i] = (U8)(((U32)(g_sGun_data[GUN_A].nDC_positive_temp+60) >> 8U) & 0xffU);
//	i++;
//	pucDst[i] = (U8)((U32)(g_sGun_data[GUN_A].nDC_positive_temp+60) & 0xffU);
//	i++;
//	//DC-温度
//	pucDst[i] = (U8)(((U32)(g_sGun_data[GUN_A].nDC_negative_temp+60) >> 8U) & 0xffU);
//	i++;
//	pucDst[i] = (U8)((U32)(g_sGun_data[GUN_A].nDC_negative_temp+60) & 0xffU);
//	i++;
//	//回枪监测
...
//	i++;
//	//DC+温度
//	pucDst[i] = (((g_sGun_data[GUN_B].nDC_positive_temp+60) >> 8) & 0xffU);
//	i++;
//	pucDst[i] = ((g_sGun_data[GUN_B].nDC_positive_temp+60) & 0xffU);
//	i++;
//	//DC-温度
//	pucDst[i] = (((g_sGun_data[GUN_B].nDC_negative_temp+60) >> 8) & 0xffU);
//	i++;
//	pucDst[i] = ((g_sGun_data[GUN_B].nDC_negative_temp+60) & 0xffU);
//	i++;
//	//回枪监测
...
//	(void)memset(g_sCommBuffs.ucResponseBuff, 0, sizeof(g_sCommBuffs.ucResponseBuff));
//
//	g_sCommBuffs.uiResponseLength = CCU_Data8027(g_sCommBuffs.ucResponseBuff);
//
//	MessageDebug(g_sCommBuffs.ucResponseBuff, g_sCommBuffs.uiResponseLength);
//
//	(void)TcpSend(g_sCommBuffs.ucResponseBuff, g_sCommBuffs.uiResponseLength);
...
static void CCU_Send8032(void)
{
	...
	g_sCommBuffs.uiResponseLength = CCU_Data8032(g_sCommBuffs.ucResponseBuff);
 ...
 * @return
 */
static void PCUStatusRestoreProcess(U8 ucGun_id)
{
	if (FALSE == GunIdValidCheck(ucGun_id))
	{
		return ;
	...
	//同步恢复禁用状态
	g_sGun_data[ucGun_id].ucGun_disable_status = GUN_ENABLE_STATUS;
	GunStatusChange(ucGun_id, STA_IDLE, TRUE, FALSE);
...
//		PileAlarmReset(ALARM_ID_PCU_ERROR);
//	}
	if (pcSrc[2] == g_sStorage_data.sPublic_data[GUN_A].ucGun_id)
	{
		//预约状态
		if (0U != pcSrc[4])
		{
		...
		{
		case 0://正常
			if (g_sGun_fault[GUN_A].sUnnecessary_upload_fault.sItem.PCU_cause_gun_error != (U8)FALSE)
			{
				GunAlarmReset(&g_sGun_fault[GUN_A].sUnnecessary_upload_fault.ucWhole_flag, BIT8_STRUCT, PCU_CAUSE_GUN_ERROR);
			}
			my_printf(USER_INFO, "%s:%d GUN_A PCU control gun normal status\n", __FILE__, __LINE__);
			PCUStatusRestoreProcess(GUN_A);
			break;
		case 1://故障
			if (g_sGun_fault[GUN_A].sUnnecessary_upload_fault.sItem.PCU_cause_gun_error != (U8)TRUE)
			{
				GunAlarmSet(&g_sGun_fault[GUN_A].sUnnecessary_upload_fault.ucWhole_flag, BIT8_STRUCT, PCU_CAUSE_GUN_ERROR);
				my_printf(USER_ERROR, "%s:%d GUN_A PCU control gun alarm status\n", __FILE__, __LINE__);
			}
			break;
		case 2://不可用
			//A枪:充电中不影响本次充电
			...
			{
				my_printf(USER_ERROR, "%s:%d CHARGING: GUN_A receive PCU control gun unavailable\n", __FILE__, __LINE__);
			}
			//A枪:设置枪不可用，禁止充电
			else
			{
				...
				my_printf(USER_ERROR, "%s:%d GUN_A PCU control gun unavailable\n", __FILE__, __LINE__);
				//同步禁用状态
				g_sGun_data[GUN_A].ucGun_disable_status = GUN_DISABLE_STATUS;
				GunStatusChange(GUN_A, STA_UNAVAILABLE, FALSE, FALSE);
			}
			break;
		default:
			my_printf(USER_ERROR, "%s:%d GUN_A PCU 0032 control cmd error = %d\n", __FILE__, __LINE__, pcSrc[3]);
			break;
		}
	}
	else if (pcSrc[2] == g_sStorage_data.sPublic_data[GUN_B].ucGun_id)
	{
		//预约状态
		if (0U != pcSrc[4])
		{
		...
		{
		case 0://正常
			if (g_sGun_fault[GUN_B].sUnnecessary_upload_fault.sItem.PCU_cause_gun_error != (U8)FALSE)
			{
				GunAlarmReset(&g_sGun_fault[GUN_B].sUnnecessary_upload_fault.ucWhole_flag, BIT8_STRUCT, PCU_CAUSE_GUN_ERROR);
			}
			PCUStatusRestoreProcess(GUN_B);
			break;
		case 1://故障
			if (g_sGun_fault[GUN_B].sUnnecessary_upload_fault.sItem.PCU_cause_gun_error != (U8)TRUE)
			{
				GunAlarmSet(&g_sGun_fault[GUN_B].sUnnecessary_upload_fault.ucWhole_flag, BIT8_STRUCT, PCU_CAUSE_GUN_ERROR);
				my_printf(USER_ERROR, "%s:%d GUN_B PCU control gun alarm status\n", __FILE__, __LINE__);
			}
			break;
		case 2://不可用
			//B枪:充电中不影响本次充电
			...
			{
				my_printf(USER_ERROR, "%s:%d CHARGING: GUN_B receive PCU control gun unavailable\n", __FILE__, __LINE__);
			}
			//B枪:设置枪不可用，禁止充电
			else
			{
				...
				my_printf(USER_ERROR, "%s:%d GUN_B PCU control gun unavailable\n", __FILE__, __LINE__);
				//同步禁用状态
				g_sGun_data[GUN_B].ucGun_disable_status = GUN_DISABLE_STATUS;
				GunStatusChange(GUN_B, STA_UNAVAILABLE, FALSE, FALSE);
			}
			break;
		default:
			my_printf(USER_ERROR, "%s:%d GUN_B PCU 0032 control cmd error = %d\n", __FILE__, __LINE__, pcSrc[3]);
			break;
		}
	}
	else
...
static void CCU_Send8034(void)
{
	...
	g_sCommBuffs.uiResponseLength = CCU_Data8034(g_sCommBuffs.ucResponseBuff);
...
static void CCU_Send8035(BOOL bFlag)
{
	...
	g_sCommBuffs.uiResponseLength = CCU_Data8035(g_sCommBuffs.ucResponseBuff, bFlag);
...
static void AnalysisMsgbody0035(U16 unMsg_len, const U8 *pcSrc)
{
	...
	if (DEBUG_MODE == g_sPile_data.ucPile_config_mode)
	{
		...
		i++;
		//A枪继电器控制
		RelayControl_A(POSITIVE_RELAY, (BOOL)pcSrc[i++]);
		vTaskDelay(10);
		RelayControl_A(NEGATIVE_RELAY, (BOOL)pcSrc[i++]);
		vTaskDelay(10);
		RelayControl_A(PRECHARGE_RELAY, (BOOL)pcSrc[i++]);
		vTaskDelay(10);
		//只支持同时关闭辅源、断开枪锁继电器
		...
		//B枪继电器控制
		RelayControl_B(POSITIVE_RELAY, (BOOL)pcSrc[i++]);
		vTaskDelay(10);
		RelayControl_B(NEGATIVE_RELAY, (BOOL)pcSrc[i++]);
		vTaskDelay(10);
		RelayControl_B(PRECHARGE_RELAY, (BOOL)pcSrc[i++]);
		//只支持同时关闭辅源、断开枪锁继电器
		if ((FALSE == (BOOL)pcSrc[i]) && (FALSE == (BOOL)pcSrc[i+1]))
		{
 ...
 * @return 无
 */
static void GUNWarnInjection(U8 ucFlag, U8 ucGun_id, U32 uiIndex)
{
	if (FALSE == GunIdValidCheck(ucGun_id))
	{
		return ;
	...
	if (uiIndex < 16U)
	{
		if (1U == ucFlag)
		{
			GunAlarmSet(&g_sGun_warn[ucGun_id].sRecoverable_warn.uiWhole_flag, BIT32_STRUCT, uiIndex);
		}
		else
		{
			GunAlarmResetTest(&g_sGun_warn[ucGun_id].sRecoverable_warn.uiWhole_flag, BIT32_STRUCT, uiIndex);
		}
	}
}
 ...
 * @return 无
 */
static void GUNFaultInjection(U8 ucFlag, U8 ucGun_id, U32 uiIndex)
{
	if (FALSE == GunIdValidCheck(ucGun_id))
	{
		return ;
	...
	if (1U == ucFlag)
	{
		...
		{
			GunAlarmSet(&g_sGun_fault[ucGun_id].sRecoverable_fault.uiWhole_flag, BIT32_STRUCT, uiIndex);
		}
		else if ((uiIndex >= 32U) && (uiIndex < 64U))//拔枪不可恢复故障
		{
			GunAlarmSet(&g_sGun_fault[ucGun_id].sGeneral_fault.uiWhole_flag, BIT32_STRUCT, uiIndex-32U);
		}
		else if ((uiIndex >= 64U) && (uiIndex < 128U))//secc故障
		{
			GunAlarmSet(&g_sSecc_fault[ucGun_id].sSecc_fault_first.u64Whole_flag, BIT64_STRUCT, uiIndex-64U);
		}
		else if ((uiIndex >= 128U) && (uiIndex < 192U))//secc故障
		{
			GunAlarmSet(&g_sSecc_fault[ucGun_id].sSecc_fault_second.u64Whole_flag, BIT64_STRUCT, uiIndex-128U);
		}
		else if ((uiIndex >= 192U) && (uiIndex < 224U))//secc故障
		{
			GunAlarmSet(&g_sSecc_fault[ucGun_id].sSecc_fault_third.uiWhole_flag, BIT32_STRUCT, uiIndex-192U);
		}
		else if (uiIndex >= 224U && uiIndex < 288U)//secc故障
		{
			GunAlarmSet(&g_sSecc_fault[ucGun_id].sSecc_fault_fourth.u64Whole_flag, BIT64_STRUCT, uiIndex-224U);
		}
		else if (uiIndex >= 288U && uiIndex < 320U)//secc故障
		{
			GunAlarmSet(&g_sSecc_fault[ucGun_id].sSecc_fault_fifth.unWhole_flag, BIT32_STRUCT, uiIndex-288U);
		}
		else
		{
			//
	...
	else
	{
		if (uiIndex < 32U)//拔枪可恢复故障
		{
			GunAlarmResetTest(&g_sGun_fault[ucGun_id].sRecoverable_fault.uiWhole_flag, BIT32_STRUCT, uiIndex);
		}
		else if ((uiIndex >= 32U) && (uiIndex < 64U))//拔枪不可恢复故障
		{
			GunAlarmResetTest(&g_sGun_fault[ucGun_id].sGeneral_fault.uiWhole_flag, BIT32_STRUCT, uiIndex-32U);
		}
		else if ((uiIndex >= 64U) && (uiIndex < 128U))
		{
			GunAlarmResetTest(&g_sSecc_fault[ucGun_id].sSecc_fault_first.u64Whole_flag, BIT64_STRUCT, uiIndex-64U);
		}
		else if ((uiIndex >= 128U) && (uiIndex < 192U))
		{
			GunAlarmResetTest(&g_sSecc_fault[ucGun_id].sSecc_fault_second.u64Whole_flag, BIT64_STRUCT, uiIndex-128U);
		}
		else if ((uiIndex >= 192U) && (uiIndex < 224U))
		{
			GunAlarmResetTest(&g_sSecc_fault[ucGun_id].sSecc_fault_third.uiWhole_flag, BIT32_STRUCT, uiIndex-192U);
		}
		else if ((uiIndex >= 224U) && (uiIndex < 288U))
		{
			GunAlarmResetTest(&g_sSecc_fault[ucGun_id].sSecc_fault_fourth.u64Whole_flag, BIT64_STRUCT, uiIndex-224U);
		}
		else if ((uiIndex >= 288U) && (uiIndex < 320U))
		{
			GunAlarmResetTest(&g_sSecc_fault[ucGun_id].sSecc_fault_fifth.unWhole_flag, BIT32_STRUCT, uiIndex-288U);
		}
		else
		{
			//
...
static BOOL AnalysisMsgHead(U8 *pucBuf, U16 unBuf_len)
{
	...
	if ((HEAD_NUM_H == pucBuf[0]) && (HEAD_NUM_L == pucBuf[1]))
	{
		...
		{
			g_sLocalState.psMsgHead = (MessageHead_t *)(&pucBuf[2]);

			if (NULL != g_sLocalState.psMsgHead)
			{
				if (unBuf_len == (g_sLocalState.psMsgHead->unMsg_len + MSGBODY_OFFSET))
				{
					return TRUE;
				}
				else
...
{
	g_sCMD_msg_control.sLogin_control.bLogin_timeup_flag = TRUE;
}

static void HeartTimeoutCallback(TimerHandle_t xTimer)
{
	TcpSendControl(&g_sCMD_msg_control.sHeart_control.bHeart_timeup_flag);
}

static void StartAuthTimeoutCallback(TimerHandle_t xTimer)
{
	TcpSendControl(&g_sCMD_msg_control.sAuth_control.bAuth_timeup_flag);
}

static void ChargingTimeoutCallback(TimerHandle_t xTimer)
{
	if (0U == g_sConnect_control.ucCharging_unansNum)
	{
	...
	else
	{
		...
		if (1U == g_sConnect_control.ucCharging_unansNum/4U)
		{
			TcpSendControl(&g_sCMD_msg_control.sCharging_control.bCharging_timeup_flag);
		}
	}
	g_sConnect_control.ucCharging_unansNum++;
}

static void StopChargeTimeoutCallback(TimerHandle_t xTimer)
{
	TcpSendControl(&g_sCMD_msg_control.sStop_charge_control.bStop_charge_timeup_flag);
}

static void DeviceTimeoutCallback(TimerHandle_t xTimer)
{
	TcpSendControl(&g_sModule_msg_control.bDevice_timeup_flag);
}
...
{
	//状态报文定时器
	g_sCMD_msg_control.sHeart_control.sTimer_heart = xTimerCreate("HeartTimer",          /* Text name. */
								 HEART_TIMER_PERIOD_MS, /* Timer period. */
								 pdTRUE,             /* Enable auto reload. */
								 "HEART",                  /* ID is not used. */
								 &HeartTimeoutCallback);   /* The callback function. */

	g_sCMD_msg_control.sAuth_control.sTimer_auth = xTimerCreate("StartAuthTimer",          /* Text name. */
								 AUTH_TIMER_PERIOD_MS, /* Timer period. */
								 pdTRUE,             /* Enable auto reload. */
								 "START_AUTH",                  /* ID is not used. */
								 &StartAuthTimeoutCallback);   /* The callback function. */

//	g_sCMD_msg_control.sCharging_control.sTimer_charging = xTimerCreate("ChargingTimer",          /* Text name. */
//								 CHARGING_TIMER_PERIOD_MS, /* Timer period. */
//								 pdTRUE,             /* Enable auto reload. */
//								 "CHARGING",                  /* ID is not used. */
//								 &ChargingTimeoutCallback);   /* The callback function. */

	g_sCMD_msg_control.sStop_charge_control.sTimer_stop_charge = xTimerCreate("StopChargeTimer",  /* Text name. */
	...
	//模块报文定时器
	g_sModule_msg_control.sTimer_device = xTimerCreate("DeviceLogTimer",          /* Text name. */
...
static void TimerSingleStart(const U8 ucTimer_flag)
{
	...
	{
	case AUTH_TIMER:
		if (NULL != g_sCMD_msg_control.sAuth_control.sTimer_auth)
		{
			(void)xTimerStart(g_sCMD_msg_control.sAuth_control.sTimer_auth, 0);
		}
	break;
	case CHARGING_TIMER:
		//充电实时数据
		if (NULL != g_sCMD_msg_control.sCharging_control.sTimer_charging)
		{
			(void)xTimerStart(g_sCMD_msg_control.sCharging_control.sTimer_charging, 0);
		}
	break;
	case STOP_CHARGE_TIMER:
		if (NULL != g_sCMD_msg_control.sStop_charge_control.sTimer_stop_charge)
		{
			(void)xTimerStart(g_sCMD_msg_control.sStop_charge_control.sTimer_stop_charge, 0);
		}
	break;
	default:
		//
 ...
 * @return
 */
static void TimerSingleStop(U8 ucTimer_flag)
{
	switch (ucTimer_flag)
	{
	case AUTH_TIMER:
		if (NULL != g_sCMD_msg_control.sAuth_control.sTimer_auth)
		{
			(void)xTimerStop(g_sCMD_msg_control.sAuth_control.sTimer_auth, 0);
		}
	break;
	case CHARGING_TIMER:
		//充电实时数据
		if (NULL != g_sCMD_msg_control.sCharging_control.sTimer_charging)
		{
			(void)xTimerStop(g_sCMD_msg_control.sCharging_control.sTimer_charging, 0);
		}
	break;
	case STOP_CHARGE_TIMER:
		if (NULL != g_sCMD_msg_control.sStop_charge_control.sTimer_stop_charge)
		{
			(void)xTimerStop(g_sCMD_msg_control.sStop_charge_control.sTimer_stop_charge, 0);
		}
	break;
	default:
		//
...
{
	//心跳
	if (NULL != g_sCMD_msg_control.sHeart_control.sTimer_heart)
	{
		(void)xTimerStart(g_sCMD_msg_control.sHeart_control.sTimer_heart, 0);
	}
	//外设
	if (NULL != g_sModule_msg_control.sTimer_device)
	{
		(void)xTimerStart(g_sModule_msg_control.sTimer_device, 0);
	}
}
...
static void TimerStop(void)
{
	if (NULL != g_sCMD_msg_control.sHeart_control.sTimer_heart)
    {
		(void)xTimerStop(g_sCMD_msg_control.sHeart_control.sTimer_heart, 0);
    }

	if (NULL != g_sModule_msg_control.sTimer_device)
	{
		(void)xTimerStop(g_sModule_msg_control.sTimer_device, 0);
	}

	TimerSingleStop(AUTH_TIMER);
	//TimerSingleStop(CHARGING_TIMER);
	g_sCMD_msg_control.sCharging_control.bCharging_timeup_flag = FALSE;
...
{
	//外设定时器计时完成
	if (g_sModule_msg_control.bDevice_timeup_flag)
	{
		//绝缘检测
		CCU_Send0005();
		//电表
		...
		g_sModule_msg_control.bDevice_timeup_flag = FALSE;
	}
}
...
static BOOL TcpDisconnect(void)
{
	if (-1 == sockfd)
	{
		g_sConnect_control.ucHeart_unansNum = 0;
		PileAlarmSet(ALARM_ID_PCU_COMM_LOST_ERROR);
		my_printf(USER_ERROR, "%s:%d server disconnect: sockfd = %d\n", __FILE__, __LINE__, sockfd);
		g_sConnect_control.bLoginResult = FALSE;

		//充电中掉线停充
		if ((U8)STA_CHARGING == g_sGun_data[GUN_A].ucGun_common_status)
		{
			g_sGun_data[GUN_A].sTemp.ucStop_Charge_type = FAULT_STOP_MODE;
			StopChargeControl(GUN_A);
		}
		if ((U8)STA_CHARGING == g_sGun_data[GUN_B].ucGun_common_status)
		{
			g_sGun_data[GUN_B].sTemp.ucStop_Charge_type = FAULT_STOP_MODE;
			StopChargeControl(GUN_B);
		}
		...
		TimerStop();
		//重新开启登录定时器
		if (NULL != g_sCMD_msg_control.sLogin_control.sTimer_login)
		{
			(void)xTimerStart(g_sCMD_msg_control.sLogin_control.sTimer_login, 0);
		}
...
{
	//心跳未收到响应报文次数超限制
	if ((g_sConnect_control.ucHeart_unansNum > HEART_UNANS_CNT_LIMIT))
	{
		g_sConnect_control.ucHeart_unansNum = 0;
		my_printf(USER_ERROR, "%s:%d login timeout:ucHeart_unansNum = %d\n", __FILE__, __LINE__, g_sConnect_control.ucHeart_unansNum);
		g_sConnect_control.bLoginResult = FALSE;
...
{
	//充电实时数据未收到响应报文次数超限制
	if (g_sConnect_control.ucCharging_unansNum > CHARGINF_UNANS_CNT_LIMIT)
	{
		g_sConnect_control.ucCharging_unansNum = 0;
...
{
	//鉴权数据未收到响应报文次数超限制
	if (g_sConnect_control.ucAuth_unansNum > AUTH_UNANS_CNT_LIMIT)
	{
		g_sConnect_control.ucAuth_unansNum = 0;
		//告知HMI
		if(NULL != g_psHMI_data)
		{
			g_psHMI_data->ucAuthorize_flag[g_psHMI_data->ucGun_id] = AUTHORIZE_FAILED;
		}
		TimerSingleStop(AUTH_TIMER);
...
{
	//鉴权数据
	if ((TRUE == g_sGun_id_status.bSend_auth_flag)
			|| (TRUE == g_sCMD_msg_control.sAuth_control.bAuth_timeup_flag))
	{
		g_sGun_id_status.bSend_auth_flag = FALSE;
		g_sCMD_msg_control.sAuth_control.bAuth_timeup_flag = FALSE;
		CCU_Send0003();
		g_sConnect_control.ucAuth_unansNum++;
		TimerSingleStart(AUTH_TIMER);
		my_printf(USER_INFO, "send 0003 auth message:bSend_auth_flag = %d bAuth_timeup_flag = %d\n",
				g_sGun_id_status.bSend_auth_flag, g_sCMD_msg_control.sAuth_control.bAuth_timeup_flag);
	}
}
...
{
	//充电实时数据
//	if ((TRUE == g_sCMD_msg_control.sCharging_control.bCharging_timeup_flag)
//			|| (TRUE == g_sCMD_msg_control.sCharging_control.bSend_charging_data_flag))
//	{
	if (TRUE == g_sCMD_msg_control.sCharging_control.bSend_charging_data_flag)
	{
		CCU_Send0004();
		//g_sCMD_msg_control.sCharging_control.bCharging_timeup_flag = FALSE;
		g_sCMD_msg_control.sCharging_control.bSend_charging_data_flag = FALSE;
...
{
	//桩停止充电请求
	if ((TRUE == g_sGun_id_status.bStop_charge_flag)
			|| (TRUE == g_sCMD_msg_control.sStop_charge_control.bStop_charge_timeup_flag))
	{
		g_sGun_id_status.bStop_charge_flag = FALSE;
		g_sCMD_msg_control.sStop_charge_control.bStop_charge_timeup_flag = FALSE;
		CCU_Send0011();
		...
		my_printf(USER_INFO, "send 0011 stop charge: bStop_flag = %d timeup_flag = %d Astop_type = %d fault code = %d %d Bstop_type = %d fault code = %d %d\n",
				g_sGun_id_status.bStop_charge_flag, g_sCMD_msg_control.sStop_charge_control.bStop_charge_timeup_flag, g_sGun_data[GUN_A].sTemp.ucStop_Charge_type,
				g_sGun_fault[GUN_A].sGeneral_fault.uiWhole_flag, g_sGun_fault[GUN_A].sRecoverable_fault.uiWhole_flag,
				g_sGun_data[GUN_B].sTemp.ucStop_Charge_type, g_sGun_fault[GUN_B].sGeneral_fault.uiWhole_flag, g_sGun_fault[GUN_B].sRecoverable_fault.uiWhole_flag);
	}
}
...
{
	if (TRUE == g_sCMD_msg_control.sHeart_control.bHeart_timeup_flag)
	{
		CCU_Send0002();
		(void)xTimerReset(g_sCMD_msg_control.sHeart_control.sTimer_heart, 0);
		g_sConnect_control.ucHeart_unansNum++;
		g_sCMD_msg_control.sHeart_control.bHeart_timeup_flag = FALSE;
	}
}
...
{
	if (TRUE == g_sCMD_msg_control.sCCU_log_control.bSend_log_flag)
	{
		CCU_Send0014();
		g_sCMD_msg_control.sCCU_log_control.bSend_log_flag = FALSE;
	}
}
...
void CommunicationParse(U8 *pucBuf, U16 unBuf_len)
{
	...
	if (NULL != g_sLocalState.psMsgHead)
	{
		switch (g_sLocalState.psMsgHead->unMsg_cmd)
		{
		case S_LOGIN_IN_ANS_CMD:
			g_sConnect_control.bLoginResult = TRUE;
			my_printf(USER_INFO, "receive server[login]response\n");
			AnalysisMsgbody8001(g_sLocalState.psMsgHead->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_GUN_STATUS_DATA_ANS_CMD:
			//my_printf(USER_DEBUG, "receive server[heartbeat]response\n");
			AnalysisMsgbody8002(g_sLocalState.psMsgHead->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_GUN_AUTH_ANS_CMD:
			TimerSingleStop(AUTH_TIMER);
			g_sConnect_control.ucAuth_unansNum = 0;
			g_sCMD_msg_control.sAuth_control.bAuth_timeup_flag = 0;
			//my_printf(USER_INFO, "receive server[charge auth]response\n");
			AnalysisMsgbody8003(g_sLocalState.psMsgHead->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
...
//			TimerSingleStop(FINISH_CHARGE_TIMER);
//			TimerSingleStop(CHARGING_TIMER);
//			AnalysisMsgbody8012(g_sLocalState.psMsgHead->unMsg_len, pucBuf + MSGBODY_OFFSET);
//			//g_sCMD_msg_control.sCharging_control.bSend_charging_data_flag = FALSE;
//			break;
		...
		case C_SECC_UP_LOG_ANS_CMD:
			...
			AnalysisMsgbody0015(g_sLocalState.psMsgHead->unMsg_len, pucBuf);
			break;
		case S_SERVERSTART_CHARGE_CMD:
			my_printf(USER_INFO, "receive server[server start charge]control\n");
			AnalysisMsgbody0020(g_sLocalState.psMsgHead->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_SERVERSTOP_CHARGE_CMD:
			my_printf(USER_INFO, "receive server[server stop charge]control\n");
			AnalysisMsgbody0021(g_sLocalState.psMsgHead->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_BOOK_CHARGE_CMD:
			my_printf(USER_INFO, "receive server[server book charge]control\n");
			AnalysisMsgbody0022(g_sLocalState.psMsgHead->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_AMOUNT_CMD:
			my_printf(USER_INFO, "receive server[server amount]control\n");
			AnalysisMsgbody0024(g_sLocalState.psMsgHead->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_GUN_CONTROL_CMD:
			my_printf(USER_INFO, "receive server[server control]control\n");
			AnalysisMsgbody0025(g_sLocalState.psMsgHead->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_SET_GUN_PARAM_CMD:
			my_printf(USER_INFO, "receive server[server config]control\n");
			AnalysisMsgbody0026(g_sLocalState.psMsgHead->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
//		case S_SERVER_QUERY_STATUS_CMD:
//			my_printf(USER_INFO, "receive server[server query]control\n");
//			AnalysisMsgbody0027(g_sLocalState.psMsgHead->unMsg_len, pucBuf + MSGBODY_OFFSET);
//			break;
		case S_CCU_UPDATE_CMD:
			//my_printf(USER_INFO, "receive server[ccu update]control\n");
			AnalysisMsgbody0028(g_sLocalState.psMsgHead->unMsg_len, pucBuf);
			break;
		case S_SECC_UPDATE_CMD:
			//my_printf(USER_INFO, "receive server[secc update]control\n");
			AnalysisMsgbody0029(g_sLocalState.psMsgHead->unMsg_len, pucBuf);
			break;
		case S_SECC_LOG_REQUEST_CMD:
			//my_printf(USER_INFO, "receive server[secc log request]control\n");
			AnalysisMsgbody0030(g_sLocalState.psMsgHead->unMsg_len, pucBuf);
			break;
		case S_SERVER_STATUS_CMD:
			//my_printf(USER_INFO, "receive server[server status]control\n");
			AnalysisMsgbody0032(g_sLocalState.psMsgHead->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			PileAlarmReset(ALARM_ID_PCU_COMM_LOST_ERROR);
			break;
		case S_SERVER_FAULT_INJECTION_CMD:
			my_printf(USER_INFO, "receive server[alarm set]control\n");
			AnalysisMsgbody0034(g_sLocalState.psMsgHead->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_SERVER_TEST_CONTROL_CMD:
			my_printf(USER_INFO, "receive server[test control]control\n");
			AnalysisMsgbody0035(g_sLocalState.psMsgHead->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_PRODUCTION_CONTROL_CMD:
			AnalysisMsgbody0036(g_sLocalState.psMsgHead->unMsg_len, &pucBuf[MSGBODY_OFFSET]);
			break;
		case S_CIG_UPDATE:
			AnalysisMsgbody0037(g_sLocalState.psMsgHead->unMsg_len, &pucBuf[0]);
			break;
		default:
			my_printf(USER_ERROR, "%s:%d receive tcp server msg cmd error = %04x\n", __FILE__, __LINE__, g_sLocalState.psMsgHead->unMsg_cmd);
			break;
		}
	}
}
...
static void Tcp_Send_Task(void *pvParameter)
{
	...
	while (1)
	{
		...
		if (NULL == g_sCMD_msg_control.sLogin_control.sTimer_login)
		{
			g_sCMD_msg_control.sLogin_control.sTimer_login = xTimerCreate("LoginTimer",          /* Text name. */
										 LOGIN_TIMER_PERIOD_MS, /* Timer period. */
										 pdTRUE,             		/* Enable auto reload. */
										 "LOGIN",               /* ID is not used. */
										 &LoginTimeoutCallback); /* The callback function. */
			CCU_Send0001();
			g_sConnect_control.ucLogin_unansNum++;
			if (NULL != g_sCMD_msg_control.sLogin_control.sTimer_login)
			{
				(void)xTimerStart(g_sCMD_msg_control.sLogin_control.sTimer_login, 0);
			}
		...
		//登录定时器触发
		if (TRUE == g_sCMD_msg_control.sLogin_control.bLogin_timeup_flag)
		{
			CCU_Send0001();
			g_sConnect_control.ucLogin_unansNum++;
			g_sCMD_msg_control.sLogin_control.bLogin_timeup_flag = FALSE;
		...
		//登录次数超限制重连
		if (g_sConnect_control.ucLogin_unansNum > LOGIN_UNANS_CNT_LIMIT)
		{

		}
		//收到登录响应报文
		if (TRUE == g_sConnect_control.bLoginResult)
		{
			//关闭登录定时器
			if (NULL != g_sCMD_msg_control.sLogin_control.sTimer_login)
			{
				(void)xTimerStop(g_sCMD_msg_control.sLogin_control.sTimer_login, 0);
				g_sConnect_control.ucLogin_unansNum = 0;
			}
			...
			while (1)
			{
				if (pdTRUE == xSemaphoreTake(g_sSystemSemaphores.client_upload_log_sem, portMAX_DELAY))
				{
					//心跳超时
...
static void AlarmFillFunc(void)
{
	...
	(void)memset(&g_sPile_alarm_upload_buff, 0, sizeof(g_sPile_alarm_upload_buff));
	(void)memset(&g_sGun_alarm_upload_buff[GUN_A], 0, sizeof(g_sGun_alarm_upload_buff[GUN_A]));
	(void)memset(&g_sGun_alarm_upload_buff[GUN_B], 0, sizeof(g_sGun_alarm_upload_buff[GUN_B]));
	...
	if (pdTRUE == xSemaphoreTake(g_sSystemSemaphores.alarm_mutex, portMAX_DELAY))
	{
		...
		for (U32 i = ALARM_ID_PILE_ALARM_BEGIN_FLAG+1U; i < ALARM_ID_PILE_ALARM_END_FLAG; i++)
		{
			...
			g_sPile_alarm_upload_buff.sPile_warn.ucPile_warn_buf[j] |= (U8)(((U32)iPile_alarm_flag & 0x01U) << k);
			k++;
			j += k/8U;
			if (8U == k)
			{
		...
		for (U32 i = ALARM_ID_PILE_ERROR_BEGIN_FLAG+1U; i < ALARM_ID_PILE_ERROR_END_FLAG; i++)
		{
			...
			g_sPile_alarm_upload_buff.sPile_fault.ucPile_fault_buf[j] |= (U8)(((U32)iPile_error_flag & 0x01U) << k);
			k++;
			j += k/8U;
			if (8U == k)
			{
		...
		//桩故障第一位为PCU端故障，不上传
		g_sPile_alarm_upload_buff.sPile_fault.ucPile_fault_buf[0] &= 0xFEU;
		//POS故障：第6字节
		//HMI故障：第7字节
		//RFID故障：第8字节
		...
		(void)memcpy(g_sGun_alarm_upload_buff[GUN_A].sGun_warn.ucGun_warn_buf, &g_sGun_warn[GUN_A].sRecoverable_warn.uiWhole_flag, uiWarn_len);
		//imd告警：第6字节
		//meter告警：第7字节
		//secc告警：第8字节
		...
		(void)memcpy(g_sGun_alarm_upload_buff[GUN_A].sGun_fault.ucGun_fault_buf, &g_sGun_fault[GUN_A].sRecoverable_fault.uiWhole_flag, uiFault_len);
		uiTemp = uiFault_len;
		uiFault_len = sizeof(g_sGun_fault[GUN_A].sGeneral_fault.uiWhole_flag);
		(void)memcpy(&g_sGun_alarm_upload_buff[GUN_A].sGun_fault.ucGun_fault_buf[uiTemp], &g_sGun_fault[GUN_A].sGeneral_fault.uiWhole_flag, uiFault_len);

		//绝缘检测故障:第9字节
		if (NULL != g_pImd_fault[GUN_A])
		{
			(void)memcpy(g_sGun_alarm_upload_buff[GUN_A].sGun_fault.ucImd_fault_buf, g_pImd_fault[GUN_A], 1);
		}
		//meter故障：第10字节
		if (NULL != g_pMeter_fault[GUN_A])
		{
			(void)memcpy(g_sGun_alarm_upload_buff[GUN_A].sGun_fault.ucMeter_fault_buf, g_pMeter_fault[GUN_A], 1);
		}
		//secc故障：第11-40字节
		uiFault_len = sizeof(g_sSecc_fault[GUN_A].sSecc_fault_first.u64Whole_flag);
		(void)memcpy(g_sGun_alarm_upload_buff[GUN_A].sGun_fault.ucSecc_fault_buf, &g_sSecc_fault[GUN_A].sSecc_fault_first.u64Whole_flag, uiFault_len);
		uiTemp = uiFault_len;
		uiFault_len = sizeof(g_sSecc_fault[GUN_A].sSecc_fault_second.u64Whole_flag);
		(void)memcpy(&g_sGun_alarm_upload_buff[GUN_A].sGun_fault.ucSecc_fault_buf[uiTemp], &g_sSecc_fault[GUN_A].sSecc_fault_second.u64Whole_flag, uiFault_len);
		uiTemp += uiFault_len;
		uiFault_len = sizeof(g_sSecc_fault[GUN_A].sSecc_fault_third.uiWhole_flag);
		(void)memcpy(&g_sGun_alarm_upload_buff[GUN_A].sGun_fault.ucSecc_fault_buf[uiTemp], &g_sSecc_fault[GUN_A].sSecc_fault_third.uiWhole_flag, uiFault_len);
		uiTemp += uiFault_len;
		uiFault_len = sizeof(g_sSecc_fault[GUN_A].sSecc_fault_fourth.u64Whole_flag);
		(void)memcpy(&g_sGun_alarm_upload_buff[GUN_A].sGun_fault.ucSecc_fault_buf[uiTemp], &g_sSecc_fault[GUN_A].sSecc_fault_fourth.u64Whole_flag, uiFault_len);
		uiTemp += uiFault_len;
		uiFault_len = sizeof(g_sSecc_fault[GUN_A].sSecc_fault_fifth.unWhole_flag);
		(void)memcpy(&g_sGun_alarm_upload_buff[GUN_A].sGun_fault.ucSecc_fault_buf[uiTemp], &g_sSecc_fault[GUN_A].sSecc_fault_fifth.unWhole_flag, uiFault_len);

		//B枪告警：1-5字节
		uiWarn_len = sizeof(g_sGun_warn[GUN_B].sRecoverable_warn.uiWhole_flag);
		(void)memcpy(g_sGun_alarm_upload_buff[GUN_B].sGun_warn.ucGun_warn_buf, &g_sGun_warn[GUN_B].sRecoverable_warn.uiWhole_flag, uiWarn_len);
		//imd告警：第6字节
		//meter告警：第7字节
		//secc告警：第8字节
		...
		(void)memcpy(g_sGun_alarm_upload_buff[GUN_B].sGun_fault.ucGun_fault_buf, &g_sGun_fault[GUN_B].sRecoverable_fault.uiWhole_flag, uiFault_len);
		uiTemp = uiFault_len;
		uiFault_len = sizeof(g_sGun_fault[GUN_B].sGeneral_fault.uiWhole_flag);
		(void)memcpy(&g_sGun_alarm_upload_buff[GUN_B].sGun_fault.ucGun_fault_buf[uiTemp], &g_sGun_fault[GUN_B].sGeneral_fault.uiWhole_flag, uiFault_len);

		//绝缘检测故障:第9字节
		if (NULL != g_pImd_fault[GUN_B])
		{
			(void)memcpy(g_sGun_alarm_upload_buff[GUN_B].sGun_fault.ucImd_fault_buf, g_pImd_fault[GUN_B], 1);
		}
		//meter故障：第10字节
		if (NULL != g_pMeter_fault[GUN_B])
		{
			(void)memcpy(g_sGun_alarm_upload_buff[GUN_B].sGun_fault.ucMeter_fault_buf, g_pMeter_fault[GUN_B], 1);
		}
		//secc故障：第11-40字节
		uiFault_len = sizeof(g_sSecc_fault[GUN_B].sSecc_fault_first.u64Whole_flag);
		(void)memcpy(g_sGun_alarm_upload_buff[GUN_B].sGun_fault.ucSecc_fault_buf, &g_sSecc_fault[GUN_B].sSecc_fault_first.u64Whole_flag, uiFault_len);
		uiTemp = uiFault_len;
		uiFault_len = sizeof(g_sSecc_fault[GUN_B].sSecc_fault_second.u64Whole_flag);
		(void)memcpy(&g_sGun_alarm_upload_buff[GUN_B].sGun_fault.ucSecc_fault_buf[uiTemp], &g_sSecc_fault[GUN_B].sSecc_fault_second.u64Whole_flag, uiFault_len);
		uiTemp += uiFault_len;
		uiFault_len = sizeof(g_sSecc_fault[GUN_B].sSecc_fault_third.uiWhole_flag);
		(void)memcpy(&g_sGun_alarm_upload_buff[GUN_B].sGun_fault.ucSecc_fault_buf[uiTemp], &g_sSecc_fault[GUN_B].sSecc_fault_third.uiWhole_flag, uiFault_len);
		uiTemp += uiFault_len;
		uiFault_len = sizeof(g_sSecc_fault[GUN_B].sSecc_fault_fourth.u64Whole_flag);
		(void)memcpy(&g_sGun_alarm_upload_buff[GUN_B].sGun_fault.ucSecc_fault_buf[uiTemp], &g_sSecc_fault[GUN_B].sSecc_fault_fourth.u64Whole_flag, uiFault_len);
		...
		(void)memcpy(&g_sGun_alarm_upload_buff[GUN_B].sGun_fault.ucSecc_fault_buf[uiTemp], &g_sSecc_fault[GUN_B].sSecc_fault_fifth.unWhole_flag, uiFault_len);
	...
	while (1)
	{
		...
		if (NULL != g_sAlarm_tigger_sem)
		{
			if (pdTRUE == xSemaphoreTake(g_sAlarm_tigger_sem, portMAX_DELAY))
			{
				...
				AlarmFillFunc();
				//告警/故障bit位填充
				g_sLocalState.ucAlarmFlag = SetAlarmFlagFunc();
				//立即上传心跳
				TcpSendControl(&g_sCMD_msg_control.sHeart_control.bHeart_timeup_flag);
				my_printf(USER_INFO, "trigger alarm modify:alarm flag = %d GUN_A=%d GUN_B=%d\n", g_sLocalState.ucAlarmFlag, g_sGun_data[GUN_A].ucGun_common_status,
						g_sGun_data[GUN_B].ucGun_common_status);
...
{
	g_sSystemSemaphores.tcp_send_mutex = xSemaphoreCreateMutex();
    g_sSystemSemaphores.client_upload_log_sem = xSemaphoreCreateBinary();
    g_sSystemSemaphores.alarm_mutex = xSemaphoreCreateMutex();
    g_sAlarm_tigger_sem = xSemaphoreCreateBinary();

    if ((NULL == g_sSystemSemaphores.client_upload_log_sem) || (NULL == g_sSystemSemaphores.alarm_mutex)
    		|| (NULL == g_sAlarm_tigger_sem) || (NULL == g_sSystemSemaphores.tcp_send_mutex))
    {
	...
	//实时数据上传
	(void)xTaskCreate(&Send_Charging_Data_Task, "Send_Charging_Data", 500U/4U, NULL, GENERAL_TASK_PRIO, NULL);
