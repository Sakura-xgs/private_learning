 ...
 * @return
 */
static void StartChargeStatusProcess(U8 ucGun_id)
{
	...
	//开始预充
	if (TRUE == AS_GetGunPrechargeStatus(ucGun_id))
	{
		s_bPrecharge_flag[ucGun_id] = TRUE;
	}
	//预充完成
	else if (FALSE == AS_GetGunPrechargeStatus(ucGun_id))
	{
		if (TRUE == s_bPrecharge_flag[ucGun_id])
		{
			...
			s_bPrecharge_flag[ucGun_id] = FALSE;
...
static void AS_GUNBChargeProcess(void)
{
	...
	if (s_cnt > 60U)
	{
		my_printf(USER_INFO, "GUN_A status = %d %d %d GUN_B status = %d %d %d\n", g_sCharge_status[GUN_A], g_sGun_data[GUN_A].ucGun_common_status,
				g_sGun_data[GUN_A].sTemp.ucCharge_current_step, g_sCharge_status[GUN_B], g_sGun_data[GUN_B].ucGun_common_status,
				g_sGun_data[GUN_B].sTemp.ucCharge_current_step);
		s_cnt = 0;
	}
