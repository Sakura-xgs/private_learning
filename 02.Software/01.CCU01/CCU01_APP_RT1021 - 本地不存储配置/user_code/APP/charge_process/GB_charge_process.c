 ...
 * @return
 */
static void GB_StartChargeStatusProcess(U8 ucGun_id)
{
	...
	//开始预充
	if ((U8)CONFIG_PRECHARGE == g_sGun_data[ucGun_id].sTemp.ucCharge_current_step)
	{
		g_bPrecharge_flag[ucGun_id] = TRUE;
	}
	//预充完成
	else if ((U8)CHARGING_BCL_BCS == g_sGun_data[ucGun_id].sTemp.ucCharge_current_step)
	{
		if (TRUE == g_bPrecharge_flag[ucGun_id])
		{
			g_sCharge_status[ucGun_id] = STA_CHARGING;
			GunStatusChange(ucGun_id, STA_CHARGING, FALSE, FALSE);

			g_bPrecharge_flag[ucGun_id] = FALSE;
			if (NULL != g_sStart_timing_sem[ucGun_id])
			{
				//启动计费
				(void)xSemaphoreGive(g_sStart_timing_sem[ucGun_id]);
			}
...
static void GB_GUNAChargeProcess(void)
{
	...
	if (s_cnt > 60U)
	{
		my_printf(USER_INFO, "GUN_A status = %d %d %d CC1 = %d GUN_B status = %d %d %d CC1 = %d\n", g_sCharge_status[GUN_A], g_sGun_data[GUN_A].ucGun_common_status,
				g_sGun_data[GUN_A].sTemp.ucCharge_current_step, g_sGun_data[GUN_A].unCC1_vol, g_sCharge_status[GUN_B], g_sGun_data[GUN_B].ucGun_common_status,
				g_sGun_data[GUN_B].sTemp.ucCharge_current_step, g_sGun_data[GUN_B].unCC1_vol);
		s_cnt = 0;
	}

	AlarmStatusProcess(GUN_A);

	switch(g_sCharge_status[GUN_A])
	{
	case STA_IDLE:
	...
	case STA_CHARG_FINISHED:
		...
		if ((U8)UNPLUGGED == GetGunStatus(GUN_A))
		{
			...
			g_sCharge_status[GUN_A] = STA_IDLE;
...
static void GB_GUNBChargeProcess(void)
{
	...
	case STA_CHARG_FINISHED:
		...
		if ((U8)UNPLUGGED == GetGunStatus(GUN_B))
		{
			...
			g_sCharge_status[GUN_B] = STA_IDLE;
