...
BOOL g_bStart_charge_flag[2] = {FALSE, FALSE};
...
Charge_Sta_t g_sCharge_status[2] = {STA_IDLE, STA_IDLE};
...
{
	if (ucGun_id >= GUN_MAX_NUM)
	{
		my_printf(USER_ERROR, "%s:%d ucGun_id = %d > 1\n", __FILE__, __LINE__);
		return FALSE;
	}
...
BOOL CheckEVStop(const U8 ucGun_id)
{
	...
	switch (g_sGun_data[ucGun_id].sTemp.ucStop_Charge_type)
	{
	case EV_DISCON_MODE:
		break;
...
void IdleStatusProcess(const U8 ucGun_id)
{
	...
	//清除充电数据
	if (FALSE == s_bClear_flag[ucGun_id])
	{
		ClearChargingData(ucGun_id);
		s_bClear_flag[ucGun_id] = TRUE;
	...
	{
		s_bClear_flag[ucGun_id] = FALSE;
		my_printf(USER_INFO, "%s:%d STA_IDLE: %s connection detected\n", __FILE__, __LINE__, (ucGun_id==GUN_A)?"GUN_A":"GUN_B");
		g_sCharge_status[ucGun_id] = STA_PLUGING;
		GunStatusChange(ucGun_id, STA_PLUGING, FALSE, FALSE);
	}
	else
	{
		if ((U8)STA_IDLE != g_sGun_data[ucGun_id].ucGun_common_status)
		{
			my_printf(USER_INFO, "%s:%d %s set STA_IDLE\n", __FILE__, __LINE__, (ucGun_id==GUN_A)?"GUN_A":"GUN_B");
			GunStatusChange(ucGun_id, STA_IDLE,	FALSE, FALSE);
		}
	}
}
 ...
 * @return
 */
static void PlugStartCharge(U8 ucGun_id)
{
	if (FALSE == GunIdValidCheck(ucGun_id))
	{
		return ;
	...
	g_sGun_data[ucGun_id].sTemp.ucStart_Charge_type = PNC_START_MODE;
	g_sGun_id_status.ucStart_control_id = ucGun_id+1U;
	g_sGun_data[ucGun_id].sTemp.ucAuth_type = START_CHARGE_AUTH;
...
void PlugingStatusProcess(const U8 ucGun_id)
{
	...
	if ((U8)UNPLUGGED == GetGunStatus(ucGun_id))
	{
		...
		g_sCharge_status[ucGun_id] = STA_IDLE;
		GunStatusChange(ucGun_id, STA_IDLE, FALSE, FALSE);
	}
	else
	{
#ifdef  ENABLE_PNC
		//即插即充使用
		PlugStartCharge(ucGun_id);
		g_sCharge_status[ucGun_id] = STA_START_CHARGE;
		g_bStart_charge_flag[ucGun_id] = FALSE;
		my_printf(USER_INFO, "%s:%d STA_PLUGING: %s start charge\n", __FILE__, __LINE__, (ucGun_id==GUN_A)?"GUN_A":"GUN_B");
#else
		//接受到充电启动标志
		if (g_bStart_charge_flag[ucGun_id])
		{
			g_sCharge_status[ucGun_id] = STA_START_CHARGE;
			g_bStart_charge_flag[ucGun_id] = FALSE;
			my_printf(USER_INFO, "%s:%d STA_PLUGING: %s start charge\n", __FILE__, __LINE__, (ucGun_id==GUN_A)?"GUN_A":"GUN_B");
...
void ChargingStatusProcess(const U8 ucGun_id)
{
	...
	{
		g_sCharge_status[ucGun_id] = STA_FAULT;
		my_printf(USER_INFO, "%s:%d STA_CHARGING:check %s error\n", __FILE__, __LINE__, (ucGun_id==GUN_A)?"GUN_A":"GUN_B");
	}
	else
	{
		//检测到结束id
		if (g_sGun_data[ucGun_id].ucGun_common_status == (U8)STA_CHARG_FINISHED)
		{
			g_sCharge_status[ucGun_id] = STA_CHARG_FINISHED;
...
void FaultStatusProcess(const U8 ucGun_id)
{
	...
	if ((U8)UNPLUGGED == GetGunStatus(ucGun_id))
	{
		...
		if (FALSE == s_bRestore_flag[ucGun_id])
		{
			//拔枪置位可恢复故障
			GunReturnResetAlarm(ucGun_id);
			s_bRestore_flag[ucGun_id] = TRUE;
		}
		//拔枪可恢复故障
		if (TRUE == RecoverableFaultCodeCheck(ucGun_id))
		{
			...
			s_bRestore_flag[ucGun_id] = FALSE;
		}
	}
}
...
void GunStatusChange(U8 ucGun_id, Charge_Sta_t ucGun_status, BOOL bUnavailable_restore_flag, BOOL bBook_restore_flag)
{
	...
	{
		//状态相同
		if ((U8)ucGun_status == g_sGun_data[ucGun_id].ucGun_common_status)
		{
		...
		//故障状态码是0xFF
		if ((U8)STA_FAULT == g_sGun_data[ucGun_id].ucGun_common_status)
		{
			ucCurrent_index = STA_MAX-1;
		}
		else
		{
			if (g_sGun_data[ucGun_id].ucGun_common_status < STA_MAX)
			{
				ucCurrent_index = g_sGun_data[ucGun_id].ucGun_common_status;
			}
			else
			{
				ucCurrent_index = STA_MAX-1;
		...
		switch ((Charge_Sta_t)ucGun_status)
		{
		case STA_FAULT:
			if (g_sGun_data[ucGun_id].ucGun_common_status != (U8)STA_FAULT)
			{
				g_sGun_data[ucGun_id].ucGun_common_status = (U8)STA_FAULT;
				my_printf(USER_INFO, "%s:%d %s status %s -> STA_FAULT\n", __FILE__, __LINE__, (ucGun_id==GUN_A)?"GUN_A":"GUN_B", s_pcGun_status_name[ucCurrent_index]);
			}
			break;
		case STA_UNAVAILABLE:
			if (g_sGun_data[ucGun_id].ucGun_common_status != (U8)STA_FAULT)
			{
				g_sGun_data[ucGun_id].ucGun_common_status = (U8)STA_UNAVAILABLE;
				my_printf(USER_INFO, "%s:%d %s status %s -> STA_UNAVAILABLE\n", __FILE__, __LINE__, (ucGun_id==GUN_A)?"GUN_A":"GUN_B", s_pcGun_status_name[ucCurrent_index]);
			}
			else
			{
				//判定桩本身是否有故障
				if (PileFaultCodeCheck(ucGun_id))
				{
		...
		case STA_BOOK:
			//空闲状态可预约
			if (g_sGun_data[ucGun_id].ucGun_common_status == (U8)STA_IDLE)
			{
				g_sGun_data[ucGun_id].ucGun_common_status = (U8)STA_BOOK;
		...
		default:
			...
			{
				if (g_sGun_data[ucGun_id].ucGun_common_status == (U8)STA_UNAVAILABLE)
				{
					g_sGun_data[ucGun_id].ucGun_common_status = (U8)STA_IDLE;
					g_sCharge_status[ucGun_id] = STA_IDLE;
			...
			{
				if (g_sGun_data[ucGun_id].ucGun_common_status == (U8)STA_BOOK)
				{
					g_sGun_data[ucGun_id].ucGun_common_status = (U8)STA_IDLE;
					g_sCharge_status[ucGun_id] = STA_IDLE;
			...
			else
			{
				...
				{
					//故障状态码是0xFF
					if (STA_FAULT == ucGun_status)
					{
						ucTarget_index = STA_MAX-1;
					}
					else
					{
						if ((U8)ucGun_status < STA_MAX)
						{
							ucTarget_index = (U8)ucGun_status;
						}
						else
						{
							ucTarget_index = STA_MAX-1;
					...
					if (STA_IDLE == ucGun_status)
					{
						//充电状态不直接置位空闲
						if (FALSE == CheckChargingStatus(ucGun_id))
						{
							g_sGun_data[ucGun_id].ucGun_common_status = (U8)ucGun_status;
...
void AlarmStatusProcess(const U8 ucGun_id)
{
	...
	static Alarm_Status_t s_ucError_status[2] = {0};

	//故障检测
	s_ucError_status[ucGun_id] = FaultCodeCheck(ucGun_id);

	switch (s_ucError_status[ucGun_id])
	{
	case NORMAL_STATUS:
		//恢复静态故障
		if (STA_FAULT == g_sCharge_status[ucGun_id])
		{
			...
			if (FALSE == CheckChargingStatus(ucGun_id))
			{
				...
				if ((U8)STA_IDLE != g_sGun_data[ucGun_id].ucGun_common_status)
				{
					my_printf(USER_INFO, "%s:%d %s alarm restore STA_FAULT->STA_IDLE %d\n", __FILE__, __LINE__,
							(ucGun_id==GUN_A)?"GUN_A":"GUN_B", g_sGun_data[ucGun_id].ucGun_common_status);
					GunStatusChange(ucGun_id, STA_IDLE,	FALSE, FALSE);
				}
			}
		}
	...
	case WARNING_STATUS:
		...
		{
			//存在拔枪恢复告警
			if (TRUE == RecoverableWarnCodeCheck(ucGun_id))
			{
				//拔枪置位可恢复故障
				GunReturnResetAlarm(ucGun_id);
			}
...
static void ChargeProcessExec(g_psCharge_Process_t *psProcess, const U8 ucGun_id)
{
	if (FALSE == GunIdValidCheck(ucGun_id))
	{
        vTaskDelay(2000);
...
void Charge_Process_Init_Task(void * pvParameters)
{
	...
	//初始化枪状态为空闲
	g_sGun_data[0].ucGun_common_status = STA_IDLE;
	g_sGun_data[1].ucGun_common_status = STA_IDLE;
