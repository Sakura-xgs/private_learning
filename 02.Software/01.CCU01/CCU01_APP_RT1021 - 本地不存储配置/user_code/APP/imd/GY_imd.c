...
__BSS(SRAM_OC) GY_Fault_t g_sGY_fault[2] = {0};
 ...
 * @return
*/
static void GYImdControl(IMD_CONTROL_t sMode, U8 ucAddr)
{
	switch (sMode)
	{
	case IMD_VERSION:
 ...
 * @return
*/
static void GYDataParse(U8 ucGun_id)
{
	...
	//当前工作模式
	if (0U != ((g_sGY_receive_data.ucStatus[0] >> 2U) & 0x01U))
	{
		g_sGY_IMD.ucImd_mode[ucGun_id] = GY_AUTO_MODE;
	}
	else
	{
		g_sGY_IMD.ucImd_mode[ucGun_id] = GY_FIXED_MODE;
	}

	//当前状态
	g_sGY_IMD.sPublic_data[ucGun_id].bImd_data_valid = (BOOL)((g_sGY_receive_data.ucStatus[1] >> 1U) & 0x01U);
	g_sGY_IMD.sPublic_data[ucGun_id].bImd_enable_status = (BOOL)((g_sGY_receive_data.ucStatus[1] >> 2U) & 0x01U);
	g_sGY_IMD.sPublic_data[ucGun_id].bImd_checkself_status = (BOOL)((g_sGY_receive_data.ucStatus[1] >> 4U) & 0x01U);

	//母线状态
	if (0U != ((g_sGY_receive_data.ucStatus[1] >> 7U) & 0x01U))
	{
		//母线反接故障
		if ((U8)TRUE != g_sGY_fault[ucGun_id].sItem.ucImd_bus_bar_reverse)
		{
			GunAlarmSet(&g_sGY_fault[ucGun_id].ucWhole_flag, BIT8_STRUCT, IMD_BUS_BAR_REVERSE);
			my_printf(USER_ERROR, "%s:%d %s trigger IMD bus bar reverse fault\n", __FILE__, __LINE__, (ucGun_id==GUN_A)?"GUN_A":"GUN_B");
		}
	}
	else
	{
		if ((U8)FALSE != g_sGY_fault[ucGun_id].sItem.ucImd_bus_bar_reverse)
		{
			GunAlarmReset(&g_sGY_fault[ucGun_id].ucWhole_flag, BIT8_STRUCT, IMD_BUS_BAR_REVERSE);
			my_printf(USER_ERROR, "%s:%d %s restore IMD bus bar reverse fault\n", __FILE__, __LINE__, (ucGun_id==GUN_A)?"GUN_A":"GUN_B");
		}
	}
	//总线电压
	g_sGY_IMD.sPublic_data[ucGun_id].unBus_vol = (((g_sGY_receive_data.unBus_vol << 8) & 0xFF00U) | ((g_sGY_receive_data.unBus_vol >> 8) & 0xFFU));
	//空闲
	if (g_sGY_IMD.sPublic_data[ucGun_id].bImd_checkself_status && (FALSE == g_sGY_IMD.sPublic_data[ucGun_id].bImd_enable_status))
	{
		g_sGY_IMD.sPublic_data[ucGun_id].ucImd_status = IMD_CHECK_INVALID;
	}
	//自检失败
	if (FALSE == g_sGY_IMD.sPublic_data[ucGun_id].bImd_checkself_status)
	{
		g_sGY_IMD.sPublic_data[ucGun_id].ucImd_status = IMD_CHECK_FAULT;
	}
	//绝缘监测中
	if (g_sGY_IMD.sPublic_data[ucGun_id].bImd_checkself_status && g_sGY_IMD.sPublic_data[ucGun_id].bImd_enable_status
			&& (FALSE == g_sGY_IMD.sPublic_data[ucGun_id].bImd_data_valid))
	{
		g_sGY_IMD.sPublic_data[ucGun_id].ucImd_status = IMD_CHECKING;
	}
	//监测完成，数据有效
	if (g_sGY_IMD.sPublic_data[ucGun_id].bImd_checkself_status && g_sGY_IMD.sPublic_data[ucGun_id].bImd_data_valid
			&& g_sGY_IMD.sPublic_data[ucGun_id].bImd_enable_status)
	{
		g_sGY_IMD.sPublic_data[ucGun_id].ucImd_status = IMD_CHECK_VALID;
		//正极绝缘电阻
		g_sGY_IMD.sPublic_data[ucGun_id].unPositive_R = (((g_sGY_receive_data.unPositive_R << 8) & 0xFF00U) | ((g_sGY_receive_data.unPositive_R >> 8) & 0xFFU));
		//负极绝缘电阻
		g_sGY_IMD.sPublic_data[ucGun_id].unNegative_R = (((g_sGY_receive_data.unNegative_R << 8) & 0xFF00U) | ((g_sGY_receive_data.unNegative_R >> 8) & 0xFFU));
	}
}
 ...
 * @return
*/
static void ImdControlResponse(U8 ucGun_id)
{
	if (FALSE == GunIdValidCheck(ucGun_id))
	{
		return ;
	...
	//计时复位
	g_uiNow_tick_imd[ucGun_id] = xTaskGetTickCount();
	//置位解析标志位
	g_sGY_IMD.ucDevice_switch_flag = (U8)ucGun_id;
	//绝缘检测开启/关闭控制响应
...
static void ImdRequestDataParse(void)
{
	if (GY_READ_CMD == g_sGY_receive_data.ucCmd)
	{
		if (IMD_ADD_A == g_sGY_receive_data.ucSlave_addr)
		{
			...
			//置位解析标志位为A绝缘检测模块
			g_sGY_IMD.ucDevice_switch_flag = (U8)GUN_A;
		...
		else if (IMD_ADD_B == g_sGY_receive_data.ucSlave_addr)
		{
			...
			//置位解析标志位为B绝缘检测模块
			g_sGY_IMD.ucDevice_switch_flag = (U8)GUN_B;
