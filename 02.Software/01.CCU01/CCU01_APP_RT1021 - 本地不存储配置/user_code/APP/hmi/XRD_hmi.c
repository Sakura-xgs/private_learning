...
static void XRDHmiParse(const U8 *pucBuf, U16 unInput_len)
{
	...
	if ((HEAD_NUM == pucBuf[HEAD_INDEX]) && (TAIL_NUM == pucBuf[unInput_len-3U]))//帧头帧尾
	{
		...
		case CHARGE_CONTROL_DATA_LEN:
			...
			{
				if ((GUN_NUM_MIN <= g_HMI_charge_control.ucGun_id) && (g_HMI_charge_control.ucGun_id <= GUN_NUM_MAX))//枪id1-8
				{
					U8 GunId = g_HMI_charge_control.ucGun_id - ((2U * g_sStorage_data.ucPile_num) - 1U);
					//判定id是否有效
					if (GunId < GUN_MAX_NUM)
					{
						...
						case STOP_CHARGE:
							//判定与启动方式是否相同
							if (g_sGun_data[GunId].sTemp.ucStart_Charge_type == g_HMI_charge_control.ucCharge_type)
							{
								HmiStopCharge(GunId);
								Uart_Dma_Send(HMI_UART, unInput_len, pucBuf);
							}
							else
							{
								my_printf(USER_ERROR, "%s:%d receive HMI stop type error start = %d stop = %d\n", __FILE__, __LINE__,
										g_sGun_data[GunId].sTemp.ucStart_Charge_type, g_HMI_charge_control.ucCharge_type);
							}
							break;
						default:
		...
		default:
			...
			if (unCrc_data == unRecv_crc)
			{
				if ((U8)ORDER_GET_CHANNEL == pucBuf[1])
				{
					...
					if (pucBuf[4] == g_sStorage_data.sPublic_data[GUN_A].ucGun_id)
					{
...
//						(void)memcpy(g_psHMI_data->sOrder_data[GUN_A].ucOrder_data, g_HMI_set_order.ucData, sizeof(g_HMI_set_order.ucData));
					...
					else if (pucBuf[4] == g_sStorage_data.sPublic_data[GUN_B].ucGun_id)
					{
