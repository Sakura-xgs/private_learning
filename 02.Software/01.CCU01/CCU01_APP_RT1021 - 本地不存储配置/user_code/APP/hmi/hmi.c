...
void HmiStartCharge(U8 ucGun_id)
{
	...
	if ((U8)POS_START_MODE == g_HMI_charge_control.ucCharge_type)
	{
		if ((FALSE == CheckChargingStatus(ucGun_id))
			&& (g_psPOS_data->POS_Transaction[ucGun_id].swipeStartFlag == FALSE))//充电中和充电启动中不要再次启动
		{
			g_psHMI_data->ucGun_id = g_HMI_charge_control.ucGun_id;
			g_psHMI_data->ucHmi_control_type = START_CHARGE;
			g_psHMI_data->ucCharge_control_type = (U8)POS_START_MODE;
			//g_psHMI_data->ucPrepayment_amount = pucBuf[6];

			//告知pos启动充电
			g_psPOS_data->POS_Transaction[ucGun_id].bHmi_TransactionStart_flag = TRUE;
			g_psPOS_data->POS_Transaction[ucGun_id].swipeStartFlag = TRUE;//置刷卡标志，表示正在刷卡
			g_psPOS_data->POS_Transaction[ucGun_id].swipeCount = 0;//清超时时间
	...
	else if ((U8)RFID_START_MODE == g_HMI_charge_control.ucCharge_type)
	{
		...
		//使能刷卡板记时
		g_psRFID_data->ucHmi_start_timing_flag = TIMER_OPEN_STATUS;
		//使能刷卡板寻卡
...
void HmiCancelCharge(U8 ucGun_id)
{
	...
	if ((U8)POS_START_MODE == g_HMI_charge_control.ucCharge_type)
	{
		...
		g_psHMI_data->ucHmi_control_type = UKNOWN_STATUS;
		//告知POS用户取消使用POS启动
		...
		{
			g_psPOS_data->POS_Transaction[ucGun_id].bHmi_TransactionStart_flag = FALSE;
		}
		else
		{
			g_psPOS_data->POS_Transaction[ucGun_id].bHmi_Abort_flag = TRUE;
		}
		g_psPOS_data->POS_Transaction[ucGun_id].swipeStartFlag = FALSE;//清刷卡标志
		my_printf(USER_INFO, "receive HMI cancel POS start charge\n", __FILE__, __LINE__);
	}
	else if ((U8)RFID_START_MODE == g_HMI_charge_control.ucCharge_type)
	{
		//取消刷卡板定时
		g_psRFID_data->ucHmi_start_timing_flag = TIMER_CLOSE_STATUS;
		//取消刷卡板寻卡
...
void HmiStopCharge(U8 ucGun_id)
{
	...
	if ((U8)POS_STOP_MODE == g_HMI_charge_control.ucCharge_type)
	{
		...
		g_psHMI_data->ucHmi_control_type = STOP_CHARGE;
		//告知POS充电完成，发送寻卡指令
		if (NULL != g_psPOS_data)
		{
			g_psPOS_data->POS_Transaction[ucGun_id].bHmi_GetCardInfo_flag = TRUE;
		}
...
void PileStatusResponse(void)
{
	...
	//枪id
	ucTmp_data[i] = g_sStorage_data.sPublic_data[GUN_A].ucGun_id;
	i++;
	//枪类型
	...
	if ((g_sGun_data[GUN_A].ucGun_common_status != (U8)STA_FAULT) && (g_sGun_data[GUN_A].ucGun_disable_status == (U8)GUN_DISABLE_STATUS))
	{
	...
	{
		if(g_sGun_data[GUN_A].ucGun_common_status == STA_CHARG_FINISHED)//如果枪状态有过充电完成
		{
			hmi_charge_finished[GUN_A] = TRUE;
		}
		else
		{
			if((g_sGun_data[GUN_A].ucGun_common_status == STA_UNAVAILABLE) || (g_sGun_data[GUN_A].ucGun_common_status == STA_FAULT))
			{
				hmi_charge_finished[GUN_A] = FALSE;
			}
		}
		...
		{
			ucTmp_data[i] = g_sGun_data[GUN_A].ucGun_common_status;
		}
	}
	i++;
	//枪特殊状态
	ucTmp_data[i] = g_sGun_data[GUN_A].ucGun_special_status;
	i++;
	//枪禁用状态
	...
	if(g_sGun_data[GUN_A].ucGun_common_status == 4)
	{
		//预充状态
		ucTmp_data[i] = 2;
	}
	...
	i++;
	//soc
	if (STA_START_CHARGE == g_sGun_data[GUN_A].ucGun_common_status)
	{
		ucTmp_data[i] = 0U;
	}
	else
	...
	//枪id
	ucTmp_data[i] = g_sStorage_data.sPublic_data[GUN_B].ucGun_id;
	i++;
	//枪类型
	...
	//枪普通状态
	//同步禁用状态到枪状态为不可用，适配HMI
	if ((g_sGun_data[GUN_B].ucGun_common_status != (U8)STA_FAULT) && (g_sGun_data[GUN_B].ucGun_disable_status == (U8)GUN_DISABLE_STATUS))
	{
		ucTmp_data[i] = STA_UNAVAILABLE;
	...
	{
		if(g_sGun_data[GUN_B].ucGun_common_status == STA_CHARG_FINISHED)//如果枪状态有过充电完成
		{
			hmi_charge_finished[GUN_B] = TRUE;
		}
		else
		{
			if((g_sGun_data[GUN_B].ucGun_common_status == STA_UNAVAILABLE) || (g_sGun_data[GUN_B].ucGun_common_status == STA_FAULT))
			{
				hmi_charge_finished[GUN_B] = FALSE;
			}
		}
		...
		{
			ucTmp_data[i] = g_sGun_data[GUN_B].ucGun_common_status;
		}
	}
	i++;
	//枪特殊状态
	ucTmp_data[i] = g_sGun_data[GUN_B].ucGun_special_status;
	i++;
	//枪禁用状态
	...
	if(g_sGun_data[GUN_B].ucGun_common_status == 4)
	{
		//预充状态
		ucTmp_data[i] = 2;
	}
	...
	i++;
	//soc
	if (STA_START_CHARGE == g_sGun_data[GUN_B].ucGun_common_status)
	{
		ucTmp_data[i] = 0U;
	}
	else
...
void Hmi_Init_Task(void * pvParameters)
{
	...
	if (TRUE == g_sStorage_data.bUse_ad_screen)
	{
		...
		(void)xTaskCreate(&Hmi_Parse_Task, "HMI_PARSE", 700U/4U, NULL, GENERAL_TASK_PRIO, NULL);
