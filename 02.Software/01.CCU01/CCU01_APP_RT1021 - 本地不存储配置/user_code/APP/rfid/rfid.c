...
static void RfidStartCharge(void)
{
			...
			|| (g_psHMI_data->ucGun_id == g_sStorage_data.sPublic_data[GUN_B].ucGun_id))
	{
		...
		if (GunId < GUN_MAX_NUM)
		{
			...
			g_sGun_data[GunId].sTemp.ucStart_Charge_type = RFID_START_MODE;
...
static void RfidStopCharge(void)
{
			...
			|| (g_psHMI_data->ucGun_id == g_sStorage_data.sPublic_data[GUN_B].ucGun_id))
	{
		...
		{
			(void)memcpy(g_sGun_data[GunId].sTemp.ucIdtag, g_psRFID_data->ucId_tag, sizeof(g_sGun_data[GunId].sTemp.ucIdtag));
			g_sGun_data[GunId].sTemp.ucStop_Charge_type = RFID_STOP_MODE;
			g_sGun_id_status.ucFinish_control_id = g_sStorage_data.sPublic_data[GunId].ucGun_id;
...
static void RfidDataProcess(void)
{
	...
	{
		switch (g_psHMI_data->ucHmi_control_type)
		{
		case START_CHARGE:
			RfidStartCharge();
			break;
...
static void Rfid_Request_Task(void *pvParameter)
{
	...
	while(1)
	{
		...
		if (NULL != g_psRFID_data)
		{
			...
			//收到hmi启动刷卡，开始记时
			if (TIMER_OPEN_STATUS == g_psRFID_data->ucHmi_start_timing_flag)
			{
				my_printf(USER_INFO, "HMI control into search card mode\n");
				(void)xTimerStart(g_sTimer_rfid, 0);
				g_psRFID_data->ucHmi_start_timing_flag = TIMER_IDLE_STATUS;
			}
			else if (TIMER_CLOSE_STATUS == g_psRFID_data->ucHmi_start_timing_flag)
			{
