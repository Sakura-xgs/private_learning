...
static void TemperatureSample(void)
{
	...
	if (g_sPile_data.ucPile_config_mode == RELEASE_MODE)
	{
		TemperatureFanDiff = gsTemperature_GUN_T[11] - gsTemperature_GUN_T[9];

		if((gsTemperature_GUN_T[11] - TEMP_OFFSET) > 35)//出风口温度大于常温
		{
			if(TemperatureFanDiff >= 5)
			{
				RelayControlPile(FAN_RELAY, RELAY_ON);//接通风扇电源
			}
			...
			else if(TemperatureFanDiff < 5)//延时停转
			{
				...
				if(FunDelayStopCount > 5U)
				{
					...
					FunDelayStopCount = 0U;
					RelayControlPile(FAN_RELAY, RELAY_OFF);//风扇断电
				}
			}
			else
			{
...
static void CcuShutDown(void)
{
	...
	//if((POS_START_MODE == g_sGun_data[GUN_A].sTemp.eStart_Charge_type) && (STA_CHARGING == g_sGun_data[GUN_A].eGun_common_status))
	if(POS_START_MODE == g_sGun_data[GUN_A].sTemp.eStart_Charge_type)
	{
		//充电费用
	...
	//if((POS_START_MODE == g_sGun_data[GUN_B].sTemp.eStart_Charge_type) && (STA_CHARGING == g_sGun_data[GUN_B].eGun_common_status))
	if(POS_START_MODE == g_sGun_data[GUN_B].sTemp.eStart_Charge_type)
	{
		//充电费用
