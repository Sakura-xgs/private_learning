...
typedef struct
{
	...
	Comm_Unans_Stat_t sComm_Unans;
	Gun_Charge_control_t sGun_charge_control;
}CCU_Msg_Control_t;

typedef struct
{
	U8 ucAuth_type;				//鉴权类型：start/stop
	Start_Charge_Type_e eStart_Charge_type;	//启动充电类型
	Stop_Charge_Type_e eStop_Charge_type;		//结束充电类型
	U8 ucIdtag[20];				//标识id
	U32 uiStart_meter_dn;		//启动电表电量
	U32 uiStart_time;			//启动时间
	U8 ucEVCCID[6];
	Charge_Step_e eCharge_step;	//当前充电步骤
	...
	U8 ucVin[17];				//车辆VIN码
	U16 unMax_allow_cell_vol;	//单体电池最高允许电压
	U16 unBat_total_energy;		//电池标称总能量
	U8 ucBms_max_allow_temp;	//最高允许温度
	//U8 ucMax_allow_cell_temp;	//单体电池最高允许温度
	...
	U16 unMax_cell_vol_and_group;//最高单体电池电压及组号
	U16 unMax_bat_vol_num;		//最高电池电压编号
	U8 ucMax_cell_temp;			//单体电池最高温度
	U8 ucMax_bat_temp_num;		//最高温度电池编号
	U8 ucMin_cell_temp;			//单体电池最低温度
	U8 ucMin_bat_temp_num;		//最低温度电池编号
	U8 ucBat_cell_vol_status;	//单体动力电池电压状态
...
typedef struct
{
	...
	Charge_Sta_e eGun_common_status;//枪普通状态
	Charge_Special_e eGun_special_status;		//枪特殊状态
	CP_STATE_e eConnect_status;		//cp/cc1状态
...
typedef struct
{
	...
	U8 ucFan_pwm;
	U8 ucFan_relay_status;
	U8 ucHMI_relay_status;
	U8 ucPile_warn[16];
	U8 ucPile_fault[16];
	U8 ucHmi_version[16];
	U8 ucPos_version[32];
...
//	U8 ucGun_num;
	...
	U8 ucLight_on_time;			//照明灯开启时间（0-24）
	U8 ucLight_off_time;		//照明灯关闭时间（0-24）
	U8 reserve1[10];
...
extern CCU_Msg_Control_t g_sMsg_control;
