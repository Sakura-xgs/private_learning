 ...
 * Variables
 ******************************************************************************/
static U32 g_uiNow_tick_secc[GUN_MAX_NUM] = {0};
...
static void ModeControlProcess(const U8* pucBuf, const Gun_Id_e eGun_id, const U8 ComType)
{
...
static void RelayDoControlProcess(const U8* pucBuf, const Gun_Id_e eGun_id, const U8 ComType)
{
	if (FALSE == GunIdValidCheck(eGun_id))
	{
		return;
	...
	if (g_sPile_data.ucPile_config_mode == PRODUCTION_MODE)
	{
		...
		case 8:
			if (0xAAU == pucBuf[2])//闭合
			{
				RelayControlPile(FAN_RELAY, RELAY_ON);
			}
			else if (0x55U == pucBuf[2])//断开
			{
				RelayControlPile(FAN_RELAY, RELAY_OFF);
			}
			else
			{
				//不响应
...
static void RelayDiCheckProcess(const U8* pucBuf, const Gun_Id_e eGun_id, const U8 ComType)
{
	if (FALSE == GunIdValidCheck(eGun_id))
	{
		return;
...
static void TempCheckProcess(const U8* pucBuf, const Gun_Id_e eGun_id, const U8 ComType)
{
	if (FALSE == GunIdValidCheck(eGun_id))
	{
		return;
	...
	if (g_sPile_data.ucPile_config_mode == PRODUCTION_MODE)
	{
		if(ComType == CANTYPE)
		{
			...
			sFrame.dataByte0 = pucBuf[0];
			iTemp = (S32)g_sGun_data[GUN_A].nDC_positive_temp+TEMP_OFFSET;
			sFrame.dataByte1 = (U8)iTemp;
			iTemp = (S32)g_sGun_data[GUN_A].nDC_negative_temp+TEMP_OFFSET;
			sFrame.dataByte2 = (U8)iTemp;
			iTemp = (S32)g_sGun_data[GUN_B].nDC_positive_temp+TEMP_OFFSET;
			sFrame.dataByte3 = (U8)iTemp;
			iTemp = (S32)g_sGun_data[GUN_B].nDC_negative_temp+TEMP_OFFSET;
		...
		else if(ComType == ETHTYPE)
		{
			...
			EthBuf[0] = pucBuf[0];
			EthBuf[1] = (U8)(g_sGun_data[GUN_A].nDC_positive_temp+TEMP_OFFSET);
			EthBuf[2] = (U8)(g_sGun_data[GUN_A].nDC_negative_temp+TEMP_OFFSET);
			EthBuf[3] = (U8)(g_sGun_data[GUN_B].nDC_positive_temp+TEMP_OFFSET);
			EthBuf[4] = (U8)(g_sGun_data[GUN_B].nDC_negative_temp+TEMP_OFFSET);
...
static void PwmCheckProcess(const U8* pucBuf, const Gun_Id_e eGun_id, const U8 ComType)
{
	if (FALSE == GunIdValidCheck(eGun_id))
	{
		return;
	...
	if (g_sPile_data.ucPile_config_mode == PRODUCTION_MODE)
	{
		...
		case 0x06:
			...
			break;
		case 0x07:
			Fan_Pwm.dutycycle = 100U - pucBuf[2];
			break;
		default:
			my_printf(USER_ERROR, "%s:%d pucBuf[1] error:%d\n", __FILE__, __LINE__, pucBuf[1]);
			break;
...
static void SoftwareCheckProcess(const Gun_Id_e eGun_id, const U8 ComType)
{
	if (FALSE == GunIdValidCheck(eGun_id))
	{
		return;
...
static void EepromCheckProcess(const U8* pucBuf, const Gun_Id_e eGun_id, const U8 ComType)
{
	if (FALSE == GunIdValidCheck(eGun_id))
	{
		return;
...
static void WatchdogCheckProcess(const U8* pucBuf, const Gun_Id_e eGun_id, const U8 ComType)
{
	if (FALSE == GunIdValidCheck(eGun_id))
	{
		return;
...
static void DeviceAddressCheck(const U8* pucBuf, const Gun_Id_e eGun_id, const U8 ComType)
{
	if (FALSE == GunIdValidCheck(eGun_id))
	{
		return;
...
static void DeviceLedControl(const U8* pucBuf, const Gun_Id_e eGun_id, const U8 ComType)
{
	if (FALSE == GunIdValidCheck(eGun_id))
	{
		return;
...
static void DeviceSnWrite(const U8* pucBuf, const Gun_Id_e eGun_id, const U8 ComType)
{
	if (FALSE == GunIdValidCheck(eGun_id))
	{
		return;
...
static void DeviceSnRead(const U8* pucBuf, const Gun_Id_e eGun_id, const U8 ComType)
{
	if (FALSE == GunIdValidCheck(eGun_id))
	{
		return;
 ...
 * @return
 */
void ProductionProcess(const U8* pucBuf, const Gun_Id_e eGun_id, const U8 ComType)
{
	if (FALSE == GunIdValidCheck(eGun_id))
	{
		return;
	...
	case PRODUCTION_MODE_CONTROL:
		ModeControlProcess(pucBuf, eGun_id, ComType);
		break;
	case RELAY_DO_CONTROL:
		RelayDoControlProcess(pucBuf, eGun_id, ComType);
		break;
	case RELAY_DI_CHECK:
		RelayDiCheckProcess(pucBuf, eGun_id, ComType);
		break;
	case TEMP_CHECK:
		TempCheckProcess(pucBuf, eGun_id, ComType);
		break;
	case UART_COMM_CHECK:
		break;
	case PWM_CHECK:
		PwmCheckProcess(pucBuf, eGun_id, ComType);
		break;
	case SOFTWARE_CHECK:
		SoftwareCheckProcess(eGun_id, ComType);
		break;
	case EEPROM_CHECK:
		EepromCheckProcess(pucBuf, eGun_id, ComType);
		break;
	case WATCHDOG_CHECK:
		WatchdogCheckProcess(pucBuf, eGun_id, ComType);
		break;
	case DEVICE_ADDRESS_CHECK:
		DeviceAddressCheck(pucBuf, eGun_id, ComType);
		break;
	case DEVICE_SN_WRITE:
		DeviceSnWrite(pucBuf, eGun_id, ComType);
		break;
	case DEVICE_SN_READ:
		DeviceSnRead(pucBuf, eGun_id, ComType);
		break;
	case DEVICE_LED_CONTROL:
		DeviceLedControl(pucBuf, eGun_id, ComType);
		break;
	default:
...
{
	//收到secc A数据
	g_uiNow_tick_secc[GUN_A] = xTaskGetTickCount();
	//充电交互
...
{
	//收到secc B数据
	g_uiNow_tick_secc[GUN_B] = xTaskGetTickCount();
	//充电交互
...
static void ChargeCanCommCheck(const Gun_Id_e eGun_id)
{
	if (FALSE == GunIdValidCheck(eGun_id))
	{
		return;
	...
	//置位通讯超时
	if ((xTaskGetTickCount() - g_uiNow_tick_secc[eGun_id]) > DEVICE_TIMEOUT_MS)
	{
		if (g_sGun_fault[eGun_id].sGeneral_fault.sItem.secc_comm_lost != TRUE)
		{
			GunAlarmSet(&g_sGun_fault[eGun_id].sGeneral_fault.uiWhole_flag, BIT32_STRUCT, (U32)FAULT_SECC_COMM_LOST);
			my_printf(USER_ERROR, "%s:%d %s SECC com disconncet\n", __FILE__, __LINE__, (eGun_id == GUN_A)?"GUN_A":"GUN_B");
		}
	}
	else//恢复
	{
		if (g_sGun_fault[eGun_id].sGeneral_fault.sItem.secc_comm_lost != FALSE)
		{
			GunAlarmReset(&g_sGun_fault[eGun_id].sGeneral_fault.uiWhole_flag, BIT32_STRUCT, (U32)FAULT_SECC_COMM_LOST);
...
static void Charge_Can_COMM_CHECK_Task(void * pvParameters)
{
	...
	g_uiNow_tick_secc[GUN_A] = xTaskGetTickCount();
	g_uiNow_tick_secc[GUN_B] = g_uiNow_tick_secc[GUN_A];
